...
| `/mailboxes/:id` | PATCH | Finance Manager or System Admin | bank mapping, opaque secret-reference names, and legacy enable switches only | updated readiness; enablement fails until the capability is connected |
| `/mailboxes/:id/oauth/start` | POST | Finance Manager or System Admin | `{capability:"ingestion"|"delivery"}`; Delivery starts carry server-side enable intent for rolling-client compatibility | `{provider,authorization_url,expires_at,capability,intent}` with a fixed provider origin |
...
| `/mailboxes/:id/delivery/enable` | POST | Finance Manager or System Admin | exact `{}` | `enabled` immediately for a valid existing credential, otherwise `oauth_required` with a fixed provider authorization URL |
| `/mailboxes/:id/delivery/disable` | POST | Finance Manager or System Admin | exact `{}` | redacted mailbox DTO; credential is retained and pending enable/reconnect states are invalidated |
| `/mailboxes/:id/delivery/reconnect` | POST | Finance Manager or System Admin | exact `{}` | `oauth_required` with actor/mailbox/company-bound reconnect intent |
| `/oauth/:provider/callback` | GET | one-time state created by Finance Manager or System Admin | provider `code` and `state`, or bounded provider `error` and `state`; Gmail additionally permits only the exact RFC 9207 issuer `iss=https://accounts.google.com` and a syntactically bounded optional Workspace `hd` hint; browser bearer/company headers are not required | browser callbacks redirect to Mailboxes with a display-only outcome; API clients receive bounded readiness metadata; never token values |
...
The normal Delivery business action uses
`POST /mailboxes/:id/delivery/enable`. Its strict result is either
`{"outcome":"enabled","mailbox":{...}}` or
`{"outcome":"oauth_required",...OAuthStart}`. A missing Delivery secret
reference is generated by the server from the immutable mailbox UUID; the
browser never supplies a Vault name. Existing valid credentials enable without
new consent. Reconnect always records `reconnect_delivery`; first-time consent
records `enable_delivery`.
...
For Delivery enable/reconnect, the state additionally binds an exact
server-authored intent and the initiating actor. The callback claims that state
once, validates the exact send scope and renewable token, then calls one
service-only PostgreSQL finalizer that writes/rotates Vault and enables Delivery
in the same transaction. Any validation or Vault failure leaves Delivery
disabled. Browser success returns HTTP 303 to the fixed
`AUTOMATION_FRONTEND_ORIGIN` Mailboxes path; its query value is presentation
feedback only and carries no activation authority.

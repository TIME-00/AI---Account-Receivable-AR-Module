...
function assertOrdered(actual: string, fragments: string[]): void {
  ...
  for (const fragment of fragments) {
    ...
    if (next === -1) {
      throw new Error(
        `Expected ordered fragment not found after index ${previous}: ${fragment}`,
      );
    }
    previous = next;
  }
...
Deno.test("Batch 9D-C migrations define root lineage and historical bootstrap invariants", async () => {
  ...
  assertStringIncludes(
    migration022,
    "fx_decision_id IS NULL OR fx_source_category IS NULL",
  );
...
Deno.test("Batch 9D-C migration 024 hardens optional source provenance records", async () => {
  ...
  assert(
    !migration024.includes(
      "CASE WHEN v_source_category = 'CATALOG' THEN v_exchange.id ELSE NULL",
    ),
    "migration 024 must not dereference optional v_exchange RECORD in CATALOG CASE expressions",
  );
  assert(
    !migration024.includes(
      "CASE WHEN v_source_category = 'REFERENCE_SELECTED' THEN v_reference.id ELSE NULL",
    ),
    "migration 024 must not dereference optional v_reference RECORD in REFERENCE_SELECTED CASE expressions",
  );

  const functionReplacementCount =
    migration024.match(/CREATE OR REPLACE FUNCTION public\./g)?.length ?? 0;
  assert(
    functionReplacementCount === 1,
    "migration 024 should replace only fx_record_booking_decision",
  );
});

Deno.test("Batch 9D-C migration 025 narrows supersession validation without broad bypass", async () => {
  const migration025 = await read(
  ...
  assertStringIncludes(
    migration025,
    "OLD.lifecycle_status NOT IN ('Draft', 'Pending', 'Approved', 'Rejected')",
  );
  assertStringIncludes(
    migration025,
    "Superseded transition may only change lifecycle_status and updated_at",
  );
  assertStringIncludes(
    migration025,
  ...
  assertStringIncludes(
    migration025,
    "BR-FX-GOVERNANCE: receipt decision currency mismatch",
  );
  assertStringIncludes(
    migration025,
    "BR-FX-GOVERNANCE: invalid catalog source",
  );
  assertStringIncludes(
    migration025,
    "BR-FX-GOVERNANCE: invalid reference source",
  );
  assertStringIncludes(
    migration025,
    "BR-FX-GOVERNANCE: invalid supersession lineage",
  );
  assertStringIncludes(
    migration025,
    "BR-FX-GOVERNANCE: BASE_PARITY requires booked_rate = 1.0",
  );

  assert(
    !migration025.includes(
      "IF NEW.lifecycle_status = 'Superseded' THEN\n    RETURN NEW;",
    ),
  );
  assert(
    !migration025.includes(
  ...
  assert(
    !/^\s*(INSERT|UPDATE|DELETE)\s+/m.test(migration025),
    "migration 025 must not contain top-level transaction data DML",
  );

  const functionReplacementCount =
    migration025.match(/CREATE OR REPLACE FUNCTION public\./g)?.length ?? 0;
  assert(
    functionReplacementCount === 1,
    "migration 025 should replace only fx_validate_booking_rate_decision",
  );
});

Deno.test("Batch 9D-C migration 026 preserves trusted import-origin provenance", async () => {
  const migration026 = await read(
    "../../../database/026_fx_booking_decision_import_origin_provenance_fix.sql",
  );
  ...
  assertStringIncludes(
    migration026,
    "NEW.import_origin IS NULL AND NEW.supersedes_decision_id IS NOT NULL",
  );
  assertStringIncludes(
    migration026,
  ...
  assertStringIncludes(
    migration026,
    "IF p_import_origin IS NOT NULL AND jsonb_typeof(p_import_origin) <> 'object'",
  );
  ...
  assert(
    !migration026.includes("LegacyBackfilled"),
    "migration 026 must not rerun historical backfill",
  );
  assert(
    !migration026.includes("UPDATE public.invoices i"),
    "migration 026 must not rewrite existing invoice pointers",
  );
  assert(
    !migration026.includes("UPDATE public.receipts r"),
    "migration 026 must not rewrite existing receipt pointers",
  );
  assert(
    !migration026.includes("post_invoice"),
    "migration 026 must not replace posting functions",
  );
  assert(
    !migration026.includes("post_receipt"),
    "migration 026 must not replace posting functions",
  );

  const createFunctionCount =
    migration026.match(/CREATE OR REPLACE FUNCTION public\./g)?.length ?? 0;
  assert(
    createFunctionCount === 3,
    "migration 026 should replace only import-origin helper and import-aware create overloads",
  );
...
Deno.test("Batch 9D-C immutability predicate rejects protected changes outside Draft-to-Draft", async () => {
  ...
  assertStringIncludes(
    migration023,
    "OLD.fx_decision_id IS DISTINCT FROM NEW.fx_decision_id",
  );
  assertStringIncludes(
    migration023,
    "OLD.invoice_date IS DISTINCT FROM NEW.invoice_date",
  );
  assertStringIncludes(
    migration023,
...
Deno.test("Batch 9D-C import explicit rate auto-post hold is wired", async () => {
  ...
  assertStringIncludes(
    importsService,
    "Explicit imported FX rate is governed as MANUAL_OVERRIDE",
  );
...
Deno.test("Batch 9D-C imports preserve explicit FX governance fields into create inputs", async () => {
  ...
  assertStringIncludes(
    importsService,
    "hasImportValue(raw, 'fx_override_reason')",
  );
  assertStringIncludes(
    importsService,
  ...
  const validationService = await read("imports/validation-service.ts");
  const helperAssignments = validationService.match(
    /const fxGovernanceFields = importFxGovernanceFields\(raw\);/g,
  )?.length ?? 0;
  assert(
    helperAssignments === 2,
    "invoice and receipt validation must both derive import FX governance fields",
  );

  const mappedDataSpreads =
    validationService.match(/\.\.\.fxGovernanceFields/g)?.length ?? 0;
  assert(
    mappedDataSpreads >= 4,
...
Deno.test("Batch 9D-C invoice and receipt create paths use atomic governed create RPCs", async () => {
  ...
  assert(
    !receiptService.includes(
      "Receipt booking-rate governance failed and draft cleanup failed",
    ),
  );
...
Deno.test("Batch 9D-C governed FX mutation RPCs own snapshot and decision changes", async () => {
  ...
  assertStringIncludes(
    migration028,
    "PERFORM public.fx_recalculate_invoice_draft_totals",
  );
  ...
  assertStringIncludes(
    migration023,
    "current_setting('app.fx_governed_mutation', true) IS DISTINCT FROM 'on'",
  );
  assertStringIncludes(
    migration023,
...
Deno.test("Batch 9D-C approval authorization is database-derived and fail-closed", async () => {
  ...
  assertStringIncludes(
    migration023,
    "missing deviation baseline cannot be approved",
  );
  assertStringIncludes(
    migration023,
...
Deno.test("Batch 9D-C stale reference governance is enforced", async () => {
  ...
  assertStringIncludes(
    migration023,
    "stale reference decision is not postable",
  );
});

...
type IsExact<TActual, TExpected> = (<T>() => T extends TActual ? 1 : 2) extends
  (<T>() => T extends TExpected ? 1 : 2)
  ? (<T>() => T extends TExpected ? 1 : 2) extends
    (<T>() => T extends TActual ? 1 : 2) ? true
  : false
  : false;
...
class MockQuery {
  ...
  single(): Promise<{ data: MockRow | null; error: null }> {
    ...
    return Promise.resolve(
      this.execute() as { data: MockRow | null; error: null },
    );
  }
...
class MockSupabaseClient {
  ...
  constructor(
    ...
    public readonly databaseRole: "authenticated" | "service_role" | "anon" =
      "authenticated",
  ) {}
  ...
  private scopedCustomerIds(params: Record<string, unknown>): Set<unknown> {
    ...
    if (params.p_scope_mode === "company") {
      return new Set(visible.map((customer) => customer.id));
    }

    const assigned = new Set(
      (this.tables.user_customer_assignments ?? [])
        .filter((assignment) =>
    ...
    return new Set(
      visible.filter((customer) => assigned.has(customer.id)).map((customer) =>
        customer.id
      ),
    );
  }
  ...
  private matchesSearch(
    ...
    if (
      search === null || search === undefined || String(search).length === 0
    ) return true;
  ...
  private invoiceCollectionSummary(params: Record<string, unknown>): unknown {
    ...
    const rows = (this.tables.invoices ?? []).filter((row) =>
      ...
      (params.p_doc_type === null || params.p_doc_type === undefined ||
        row.doc_type === params.p_doc_type) &&
      (params.p_status === null || params.p_status === undefined ||
        row.status === params.p_status) &&
      (params.p_customer_id === null || params.p_customer_id === undefined ||
        row.customer_id === params.p_customer_id) &&
      (params.p_posting_period === null ||
        params.p_posting_period === undefined ||
        row.posting_period === params.p_posting_period) &&
      (params.p_date_from === null || params.p_date_from === undefined ||
        String(row.invoice_date) >= String(params.p_date_from)) &&
      (params.p_date_to === null || params.p_date_to === undefined ||
        String(row.invoice_date) <= String(params.p_date_to)) &&
      this.matchesSearch(row, params.p_search, [
        "invoice_no",
        "customer_name",
        "reference_no",
      ])
    ...
    const baseCurrency = String(
      (this.tables.companies ?? []).find((company) =>
        company.id === params.p_company_id
      )?.base_currency ?? "MYR",
    );
    ...
    return {
      current_balance_summary: monetarySummaryFromEntries(
        rows.map((row) => ({
          ...
          base_amount: currentBaseFromBookedRate(
  ...
  private receiptCollectionSummary(params: Record<string, unknown>): unknown {
    ...
    const rows = (this.tables.receipts ?? []).filter((row) =>
      ...
      (params.p_customer_id === null || params.p_customer_id === undefined ||
        row.customer_id === params.p_customer_id) &&
      (params.p_payment_method === null ||
        params.p_payment_method === undefined ||
        row.payment_method === params.p_payment_method) &&
      (params.p_posting_period === null ||
        params.p_posting_period === undefined ||
        row.posting_period === params.p_posting_period) &&
      (params.p_date_from === null || params.p_date_from === undefined ||
        String(row.receipt_date) >= String(params.p_date_from)) &&
      (params.p_date_to === null || params.p_date_to === undefined ||
        String(row.receipt_date) <= String(params.p_date_to)) &&
      this.matchesSearch(row, params.p_search, [
        "receipt_no",
        "customer_name",
        "reference_no",
  ...
  private invoiceCollection(params: Record<string, unknown>): unknown {
    const page = this.invoiceCollectionPage(params) as {
      rows: MockRow[];
      total: number;
    };
  ...
  private receiptCollection(params: Record<string, unknown>): unknown {
    const page = this.receiptCollectionPage(params) as {
      rows: MockRow[];
      total: number;
    };
  ...
  private agingBucketKey(
    ...
    const elapsed = Math.floor(
      (new Date(`${asOfDate}T00:00:00Z`).getTime() -
        new Date(`${String(row.due_date)}T00:00:00Z`).getTime()) / 86400000,
    );
  ...
  private agingSummary(params: Record<string, unknown>): unknown {
    ...
    const totals = new Map<
      string,
      { currency: string; amount: number; base_amount: number; count: number }
    >();
    const bucketKeys = [
      "current",
      "1_30",
    ...
    for (const row of rows) {
      ...
      const baseAmount = currentBaseFromBookedRate(
        amount,
        Number(row.exchange_rate),
      );
    ...
    return {
      ...
      aging_summary: definitions.map(([key, name, fromDays, toDays]) => {
        ...
        return {
          ...
          total_outstanding: bucket.amount,
          base_total: bucket.amount,
          base_currency: baseCurrency,
          by_currency: currencyTotalsFromMap(bucket.currencies),
          normalization_basis: CURRENT_BALANCE_BOOKED_RATE_BASIS,
          percentage: baseTotal > 0
            ? roundMoney(bucket.amount / baseTotal * 100)
            : 0,
        };
      }),
    };
  ...
  private agingByCustomer(params: Record<string, unknown>): unknown {
    ...
    for (const row of rows) {
      grouped.set(String(row.customer_id), [
        ...(grouped.get(String(row.customer_id)) ?? []),
        row,
      ]);
    }
    const customerRows = [...grouped.entries()].map(
      ([customerId, invoiceRows]) => {
        const customer = (this.tables.customers ?? []).find((row) =>
          row.id === customerId
        )!;
        ...
        return {
          ...
          bucket_1_30: amounts["1_30"],
          bucket_31_60: amounts["31_60"],
          bucket_61_90: amounts["61_90"],
          bucket_over_90: amounts.over_90,
        };
      },
    ).filter((row) =>
    ...
    const pageSize = Number(params.p_page_size);
    return {
      rows: customerRows.slice((page - 1) * pageSize, page * pageSize),
  ...
  private customerStatement(params: Record<string, unknown>): unknown {
    ...
    const opening = [
      ...invoices.filter((row) => String(row.invoice_date) < periodFrom).map(
        (row) => ({
          currency: String(row.currency),
          debit: row.doc_type === "Credit Note" ? 0 : Number(row.total_amount),
          credit: row.doc_type === "Credit Note" ? Number(row.total_amount) : 0,
          base_debit: row.doc_type === "Credit Note"
            ? 0
            : Number(row.base_total),
          base_credit: row.doc_type === "Credit Note"
            ? Number(row.base_total)
            : 0,
        }),
      ),
      ...receipts.filter((row) => String(row.receipt_date) < periodFrom).map(
        (row) => ({
          currency: String(row.currency),
          debit: 0,
          credit: Number(row.receipt_amount),
          base_debit: 0,
          base_credit: Number(row.base_amount),
        }),
      ),
    ];
    const period = [
      ...invoices.filter((row) =>
        String(row.invoice_date) >= periodFrom &&
        String(row.invoice_date) <= periodTo
      ).map((row) => ({
        id: String(row.id),
        date: String(row.invoice_date),
        doc_type: String(row.doc_type),
        doc_no: String(row.invoice_no),
        description: `${String(row.doc_type)}: ${String(row.invoice_no)}`,
        currency: String(row.currency),
      ...
      ...receipts.filter((row) =>
        String(row.receipt_date) >= periodFrom &&
        String(row.receipt_date) <= periodTo
      ).map((row) => ({
        id: String(row.id),
        date: String(row.receipt_date),
        doc_type: "Receipt",
        doc_no: String(row.receipt_no),
        description: `Receipt: ${String(row.receipt_no)} (${
      ...
      a.date.localeCompare(b.date) || a.doc_no.localeCompare(b.doc_no) ||
    ...
    const movements = [
      ...opening.map((row) => ({
        ...
        opening_delta: row.debit - row.credit,
      })),
      ...period.map((row) => ({
        currency: row.currency,
        debit: row.debit,
    ...
    let transactionBalance = opening.reduce(
      (sum, row) => roundMoney(sum + row.debit - row.credit),
      0,
    );
    let baseBalance = opening.reduce(
      (sum, row) => roundMoney(sum + row.base_debit - row.base_credit),
      0,
    );
    const lines = period.map((row) => {
      transactionBalance = roundMoney(
        transactionBalance + row.debit - row.credit,
      );
    ...
    const totalDebit = period.reduce(
      (sum, row) => roundMoney(sum + row.debit),
      0,
    );
    const totalCredit = period.reduce(
      (sum, row) => roundMoney(sum + row.credit),
      0,
    );
    const totalDebitBase = period.reduce(
      (sum, row) => roundMoney(sum + row.base_debit),
      0,
    );
    const totalCreditBase = period.reduce(
      (sum, row) => roundMoney(sum + row.base_credit),
      0,
    );
    const openingBalance = opening.reduce(
      (sum, row) => roundMoney(sum + row.debit - row.credit),
      0,
    );
    const openingBalanceBase = opening.reduce(
      (sum, row) => roundMoney(sum + row.base_debit - row.base_credit),
      0,
    );
    ...
    return {
      ...
      lines,
      closing_balance: multiCurrency
        ? null
        : roundMoney(openingBalance + totalDebit - totalCredit),
      ...
      closing_balance_base: roundMoney(
        openingBalanceBase + totalDebitBase - totalCreditBase,
      ),
      ...
      legacy_transaction_currency: multiCurrency
        ? null
        : byCurrency[0]?.currency ?? null,
    };
  }
  ...
  rpc(
    ...
    if (
      functionName.startsWith("ar_") && this.databaseRole !== "authenticated"
    ) {
      return {
        data: null,
        error: {
          code: "42501",
          message: "permission denied for authenticated read RPC",
        },
      };
    }
    if (functionName === "get_next_sequence") {
      return {
        data: params.p_doc_type === "RCT" ? "RCT-MOCK-001" : "INV-MOCK-001",
        error: null,
      };
    }
    ...
    if (functionName === "update_draft_invoice") {
      const invoice = (this.tables.invoices ?? []).find((row) =>
        row.id === params.p_invoice_id
      );
    ...
    if (functionName === "correct_posted_invoice_reference") {
      ...
      return {
        data: invoice
          ? { ...invoice, reference_no: params.p_reference_no }
          : null,
        error: null,
      };
    }
    ...
    if (functionName === "delete_draft_invoice_line") {
      ...
      return {
        data: { deleted: index >= 0, id: params.p_line_id },
        error: null,
      };
    }
    ...
    if (functionName === "cancel_receipt") {
      const receipt = (this.tables.receipts ?? []).find((row) =>
        row.id === params.p_receipt_id
      );
      return {
        data: receipt
          ? {
...
class FailingQuery extends MockQuery {
  override then<TResult1 = unknown, TResult2 = never>(
    ...
    return Promise.resolve({
      data: null,
      error: { message: "forced decision read failure" },
    })
...
Deno.test("Batch 9D-D grouped mixed-currency totals never collapse to transaction sum 300", () => {
  ...
  const baseTotal = roundMoney(
    byCurrency.reduce((sum, row) => sum + row.base_amount, 0),
  );
  ...
  assert(
    baseTotal !== 300,
    "Mixed-currency transaction amounts must not render as one monetary total",
  );
  assertEquals(meta.multi_currency, true, "Expected multi-currency metadata");
  assertEquals(
    meta.normalization_basis,
    CURRENT_BALANCE_BOOKED_RATE_BASIS,
    "Expected booked-rate current-balance basis",
  );
});
  ...
  const client = new MockSupabaseClient({
    invoices: Array.from(
      ...
      (_, index) => ({
        id: `projection-${index}`,
        base_total: index,
        omitted: "must-not-leak",
      }),
    ),
  });
  const result = await client.from("invoices").select(
    "id,base_total",
  ) as unknown as { data: MockRow[]; count: number };
...
Deno.test("Batch 9D-D current-base derivation uses current balance and immutable booked rate", () => {
  ...
  assertEquals(
    currentBaseFromBookedRate(originalTransactionTotal, bookedRate),
    412.5,
  );
...
Deno.test("Batch 9D-D monetary rounding is backend-defined at two decimals including .005 boundary", () => {
  for (
    ...
    assertEquals(
      roundMoney(input),
      expected,
      `Unexpected PostgreSQL-compatible rounding for ${input}`,
    );
  }
...
Deno.test("Batch 9D-D operational currency validation accepts only the approved new-write set", () => {
  ...
  for (const currency of ["USD", "EUR", "GBP", "CNY", "AUD", "JPY", "HKD"]) {
    ...
    throw new Error(
      `Expected unsupported currency ${currency} to fail closed for new writes`,
    );
  }
});
...
Deno.test("Batch 9D-D statement v2 groups native balances by currency and marks mixed scalar compatibility", () => {
  ...
  const unsafeMixedScalar = byCurrency.reduce(
    (sum, row) => sum + row.opening_balance,
    0,
  );
  assertEquals(unsafeMixedScalar, 300);
  assert(
    meta.multi_currency,
    "Legacy scalar transaction fields must be treated as invalid when currencies are mixed",
  );
});
...
Deno.test("Batch 9D-D invoice report summary contract returns grouped and company-base totals", () => {
  const summary = monetarySummaryFromEntries([
    {
      ...
      base_amount: currentBaseFromBookedRate(100, 1.35),
    },
    {
      currency: "SGD",
      transaction_amount: 100,
      base_amount: currentBaseFromBookedRate(100, 3.10),
    },
    {
      currency: "MYR",
      transaction_amount: 100,
      base_amount: currentBaseFromBookedRate(100, 1.00),
    },
  ], "MYR");
  ...
  assertJsonEquals(
    summary.by_currency.map(
      (row) => [row.currency, row.amount, row.base_amount],
    ),
    [
      ["MYR", 100, 100],
      ["SGD", 100, 310],
  ...
  const receiptSummary = monetarySummaryFromEntries([
    {
      ...
      base_amount: currentBaseFromBookedRate(40, 1.35),
    },
    {
      currency: "SGD",
      transaction_amount: 20,
      base_amount: currentBaseFromBookedRate(20, 3.10),
    },
  ], "MYR");
  const outstandingSummary = monetarySummaryFromEntries([
    {
      ...
      base_amount: currentBaseFromBookedRate(25, 1.35),
    },
    {
      currency: "MYR",
      transaction_amount: 75,
      base_amount: currentBaseFromBookedRate(75, 1.00),
    },
  ], "MYR");
...
Deno.test("Batch 9D-D invoice and receipt API envelopes expose one typed monetary summary at meta.summary", () => {
  ...
  const invoiceEnvelope: MonetaryCollectionAPIResponse<Invoice[]> =
    successResponse<Invoice[], MonetaryCollectionSummary>(
      [],
      { total: 1, page: 1, page_size: 50, summary },
    );
  const receiptEnvelope: MonetaryCollectionAPIResponse<Receipt[]> =
    successResponse<Receipt[], MonetaryCollectionSummary>(
      [],
      { total: 1, page: 1, page_size: 50, summary },
    );

  type InvoiceSummaryType = NonNullable<
    NonNullable<typeof invoiceEnvelope.meta>["summary"]
  >;
  type ReceiptSummaryType = NonNullable<
    NonNullable<typeof receiptEnvelope.meta>["summary"]
  >;
  const invoiceSummaryIsExact: IsExact<
    InvoiceSummaryType,
    MonetaryCollectionSummary
  > = true;
  const receiptSummaryIsExact: IsExact<
    ReceiptSummaryType,
    MonetaryCollectionSummary
  > = true;
  ...
  assertEquals(
    invoiceEnvelope.meta?.summary?.current_balance_summary.amount_basis,
    "current_outstanding",
  );
  assertEquals(
    invoiceEnvelope.meta?.summary?.document_total_summary.amount_basis,
    "original_document_total",
  );
  assertEquals(
    receiptEnvelope.meta?.summary?.current_balance_summary.base_total,
    135,
  );
  assertEquals(
    receiptEnvelope.meta?.summary?.document_total_summary.base_total,
    162,
  );
  assertJsonEquals(Object.keys(invoiceEnvelope).sort(), [
    "data",
    "meta",
    "success",
  ]);
  assert(
    invoiceEnvelope.meta && "summary" in invoiceEnvelope.meta,
    "Runtime summary must remain under meta.summary",
  );
  assert(
    receiptEnvelope.meta && "summary" in receiptEnvelope.meta,
    "Receipt runtime summary must remain under meta.summary",
  );
});
...
Deno.test("Batch 9D-D canonical API response remains compatible without collection summaries or metadata", () => {
  ...
  const withoutMetadata: APIResponse<{ id: string }> = successResponse({
    id: "row-1",
  });

  assertEquals(paginated.meta?.total, 1);
  assert(
    !paginated.meta || !("summary" in paginated.meta),
    "Ordinary paginated responses must not fabricate summaries",
  );
...
Deno.test("Batch 9D-D monetary collection summary has one canonical interface definition", async () => {
  ...
  assert(
    !sharedTypes.includes("interface MonetaryCollectionSummary"),
    "Shared envelope types must not duplicate the monetary summary interface",
  );
  assertEquals(
    monetaryContracts.match(/export interface MonetaryCollectionSummary/g)
      ?.length ?? 0,
    1,
...
Deno.test("Batch 9D-D InvoiceService.listInvoices returns scoped rows and full filtered summaries", async () => {
  ...
  assertEquals(
    ...
    1,
    "Pagination should return only one row",
  );
  assertEquals(
    result.total,
    2,
    "Count should include all filtered matching rows",
  );
  assertEquals(
    result.summary.current_balance_summary.row_count,
    2,
    "Summary should use all filtered rows, not only page rows",
  );
  assertEquals(
    result.summary.current_balance_summary.amount_basis,
    "current_outstanding",
  );
  assertEquals(result.summary.current_balance_summary.base_total, 445);
  assertEquals(
    result.summary.document_total_summary.amount_basis,
    "original_document_total",
  );
  assertEquals(result.summary.document_total_summary.base_total, 503);
  assertEquals(
    result.summary.current_balance_summary.meta.multi_currency,
    true,
  );
  assertEquals(
  ...
  const response = successResponse(result.invoices, {
    total: result.total,
    page: 1,
    page_size: 1,
  ...
  assert(
    ...
    "Index response metadata must carry a typed summary",
  );
});
...
Deno.test("Batch 9D-D single collection RPC keeps page and summary snapshot-consistent across a simulated concurrent change", async () => {
  ...
  const invoiceClient = new MockSupabaseClient({
    ...common,
    invoices: [invoice],
  });
  invoiceClient.afterCollectionSnapshot = () => {
    invoiceClient.tables.invoices.push({
      ...invoice,
      id: "inv-snapshot-2",
      invoice_no: "SNAP-INV-2",
    });
  };
  const invoiceResult = await new InvoiceService(
    invoiceClient as never,
    invoiceClient as never,
  ).listInvoices(
    clerkAuth,
    { search: "SNAP" },
  ...
  assertEquals(
    invoiceClient.rpcCalls.filter((call) =>
      call.functionName === "ar_invoice_collection"
    ).length,
    1,
  );
  ...
  const receiptClient = new MockSupabaseClient({
    ...common,
    receipts: [receipt],
  });
  receiptClient.afterCollectionSnapshot = () => {
    receiptClient.tables.receipts.push({
      ...receipt,
      id: "rct-snapshot-2",
      receipt_no: "SNAP-RCT-2",
    });
  };
  const receiptResult = await new ReceiptService(
    receiptClient as never,
    receiptClient as never,
  ).listReceipts(
    clerkAuth,
    { search: "SNAP" },
  ...
  assertEquals(
    receiptClient.rpcCalls.filter((call) =>
      call.functionName === "ar_receipt_collection"
    ).length,
    1,
  );
...
Deno.test("Batch 9D-D ordinary user-domain reads reject a service-role-only mock client", async () => {
  ...
  const collectionOperations = [
    () =>
      new InvoiceService(serviceRoleClient as never, serviceRoleClient as never)
        .listInvoices(clerkAuth, {}, { page: 1, page_size: 10 }),
    () =>
      new ReceiptService(serviceRoleClient as never, serviceRoleClient as never)
        .listReceipts(clerkAuth, {}, { page: 1, page_size: 10 }),
  ];
...
Deno.test("Batch 9D-D services route migration-027 reads through the authenticated client, not the mutation client", async () => {
  ...
  const authenticatedReadClient = new MockSupabaseClient(
    tables,
    "authenticated",
  );
  ...
  await new InvoiceService(
    ...
    authenticatedReadClient as never,
  ).listInvoices(
    clerkAuth,
    {},
    { page: 1, page_size: 10 },
  ...
  assertEquals(
    mutationClient.rpcCalls.length,
    0,
    "Migration-027 reads must not use the trusted mutation client",
  );
  assertEquals(
    authenticatedReadClient.rpcCalls.filter((call) =>
      call.functionName === "ar_invoice_collection"
    ).length,
    1,
  );
  ...
  assertEquals(
    authenticatedReadClient.rpcCalls.filter((call) =>
      call.functionName === "ar_aging_summary"
    ).length,
    1,
  );
});
...
Deno.test("Batch 9D-D authorized invoice detail succeeds for assigned Clerk and Finance Manager through read client only", async () => {
  ...
  for (const auth of [clerkAuth, managerAuth]) {
    ...
    assertEquals(
      readClient.operations.some((op) =>
        op.table === "invoices" && op.op === "select"
      ),
      true,
    );
    assertEquals(
      mutationClient.operations.length,
      0,
      "Invoice detail must never SELECT through trusted mutation client",
    );
  }
});
...
Deno.test("Batch 9D-D RLS-hidden and nonexistent invoice detail share the same authorization-safe 404 contract", async () => {
  ...
  const hiddenMutationClient = new MockSupabaseClient({
    invoices: [{
      id: invoiceId,
      company_id: "co-1",
      customer_id: "cust-unassigned",
      invoice_no: "INV-HIDDEN-001",
      doc_type: "Invoice",
      status: "Open",
      currency: "MYR",
      base_total: 10,
  ...
  const hiddenReadClient = new MockSupabaseClient(
    { invoices: [] },
    "authenticated",
  );
  const nonexistentMutationClient = new MockSupabaseClient(
    { invoices: [] },
    "service_role",
  );
  const nonexistentReadClient = new MockSupabaseClient(
    { invoices: [] },
    "authenticated",
  );
  ...
  const nonexistentError = await rejectedValue(() =>
    new InvoiceService(
      nonexistentMutationClient as never,
      nonexistentReadClient as never,
    )
      .getInvoiceById(clerkAuth, invoiceId)
  );

  assert(
    hiddenError instanceof NotFoundError,
    "RLS-hidden invoice must map to NotFoundError",
  );
  assert(
    nonexistentError instanceof NotFoundError,
    "Nonexistent invoice must map to NotFoundError",
  );
  ...
  assertEquals(
    hiddenMutationClient.operations.length,
    0,
    "Hidden lookup must not use trusted mutation client",
  );
  assertEquals(
    nonexistentMutationClient.operations.length,
    0,
    "Missing lookup must not use trusted mutation client",
  );
});
...
Deno.test("Batch 9D-D invoice detail preserves genuine PostgREST failures as HTTP 500", async () => {
  ...
  readClient.queryErrors.invoices = {
    code: "XX000",
  ...
  assert(
    ...
    "Unexpected database failure must not become NotFoundError",
  );
  ...
  assertEquals(
    mutationClient.operations.length,
    0,
    "Failed detail read must not fall back to trusted client",
  );
});
...
Deno.test("Batch 9D-D delegated Credit Note and Debit Note hidden detail paths inherit authorization-safe 404", async () => {
  ...
  assertEquals(
    mutationClient.operations.length,
    0,
    "Delegated hidden detail must not query trusted client",
  );
});
...
Deno.test("Batch 9D-D unassigned invoice collection remains an empty authorized result", async () => {
  ...
  const result = await new InvoiceService(
    ...
    .listInvoices(clerkAuth, { customer_id: "cust-unassigned" }, {
      page: 1,
      page_size: 10,
    });
...
Deno.test("Batch 9D-D generic Invoice creation accepts only a valid Linked Credit Note reference", async () => {
  const genericClient = new MockSupabaseClient(
    linkedCreditNoteTables(),
    "service_role",
  );
  ...
  assertEquals(
    genericClient.rpcCalls.filter((call) =>
      call.functionName === "fx_create_governed_invoice_draft"
    ).length,
    1,
  );

  const dedicatedClient = new MockSupabaseClient(
    linkedCreditNoteTables(),
    "service_role",
  );
  ...
  assertEquals(
    dedicatedClient.rpcCalls.filter((call) =>
      call.functionName === "fx_create_governed_invoice_draft"
    ).length,
    1,
...
Deno.test("Gate A reference-selected Invoice and Receipt creation use the exact canonical RPC arguments", async () => {
  ...
  const invoice = await new InvoiceService(client as never).createInvoice(
    clerkAuth,
    {
      doc_type: "Invoice",
      customer_id: "cust-a",
  ...
  const receipt = await new ReceiptService(client as never).createReceipt(
    clerkAuth,
    {
      customer_id: "cust-a",
      receipt_date: "2026-07-24",
...
Deno.test("Gate A Draft Receipt update service forwards reference authority and rejects invalid scope or mixed authority before RPC", async () => {
  ...
  const unauthorizedClient = new MockSupabaseClient(
    { receipts: [receipt] },
    "service_role",
  );
  ...
  assertEquals(
    (unauthorizedError as { code?: string }).code,
    "AUTHORIZATION_ERROR",
  );
...
Deno.test("Batch 9D-D Linked Credit Note creation fails closed for every invalid reference class", async () => {
  const cases: Array<{ label: string; reference: MockRow | null }> = [
    ...
    {
      label: "other company",
      reference: { company_id: "co-2", invoice_no: "SECRET-OTHER-COMPANY" },
    },
    {
      label: "other customer",
      reference: { customer_id: "cust-b", invoice_no: "SECRET-OTHER-CUSTOMER" },
    },
  ...
  for (const testCase of cases) {
    const client = new MockSupabaseClient(
      linkedCreditNoteTables(testCase.reference),
      "service_role",
    );
    const error = await rejectedValue(() =>
    ...
    assert(
      error instanceof BusinessError,
      `${testCase.label} must produce a business error`,
    );
...
Deno.test("Batch 9D-D Linked and Standalone Credit Note structural inputs fail closed before creation", async () => {
  const missingReferenceClient = new MockSupabaseClient(
    linkedCreditNoteTables(),
    "service_role",
  );
  ...
  const standaloneClient = new MockSupabaseClient(
    linkedCreditNoteTables(),
    "service_role",
  );
...
Deno.test("Batch 9D-D Migration 028 self-reference error remains a deterministic BR-CN-REF response", async () => {
  const client = new MockSupabaseClient(
    linkedCreditNoteTables({ id: "inv-created" }),
    "service_role",
  );
  client.rpcErrors.fx_create_governed_invoice_draft = {
    code: "P0001",
    message:
      "BR-CN-REF: linked credit note reference is invalid or unavailable",
  };
  ...
  assert(
    error instanceof BusinessError,
    `Expected Linked Credit Note validation error, got ${String(error)}`,
  );
  assertEquals(error.code, "BR-CN-REF");
...
Deno.test("Batch 9D-D Draft Linked Credit Note currency validation is wired before FX mutation", async () => {
  ...
  const invoiceServiceSource = await read("../invoices/lifecycle-service.ts");
  const updateStart = invoiceServiceSource.indexOf("async updateDraftInvoice(");
  const validationCall = invoiceServiceSource.indexOf(
    "await this.validateLinkedCreditNoteReference(",
    updateStart,
  );
  const governedUpdateCall = invoiceServiceSource.indexOf(
    'callRpc<Invoice>(this.client, "update_draft_invoice"',
    updateStart,
  );
  assert(
    updateStart >= 0 && validationCall > updateStart &&
      governedUpdateCall > validationCall,
...
Deno.test("Batch 9D-D Draft Invoice header update production composition uses one governed RPC", async () => {
  const source = await readInvoiceServiceSource(import.meta.url);
...
Deno.test("Batch 9D-D Invoice cancellation uses one governed atomic RPC and returns only its committed result", async () => {
  ...
  assertEquals(
    client.rpcCalls.some((call) =>
      call.functionName === "reverse_journal_entry"
    ),
    false,
  );
  assertEquals(
    client.operations.some((operation) =>
      operation.table === "allocation_details"
    ),
    false,
  );
  assertEquals(
    client.operations.some((operation) =>
      operation.table === "journal_entries"
    ),
    false,
  );

  const invoiceServiceSource = await readInvoiceServiceSource(import.meta.url);
  const cancellationStart = invoiceServiceSource.indexOf(
    "async cancelInvoice(",
  );
  const cancellationEnd = invoiceServiceSource.indexOf(
    "// GET / LIST INVOICES",
    cancellationStart,
  );
  const cancellationBody = invoiceServiceSource.slice(
    cancellationStart,
    cancellationEnd,
  );
  assert(
    cancellationBody.includes("callRpc<Invoice>(this.client, 'cancel_invoice'"),
  );
...
Deno.test("Batch 9D-D atomic Invoice cancellation maps governed business, conflict, and internal errors", async () => {
  ...
  const cases = [
    {
      error: {
        code: "P0001",
        message:
          "BR-CN-REF: Linked Credit Note reference integrity prevents this Invoice cancellation",
      },
      status: 400,
      code: "BR-CN-REF",
    },
    {
      error: {
        code: "P0001",
        message:
          "BR-INV-003: Invoice cannot be cancelled while active allocations exist",
      },
      status: 400,
      code: "BR-INV-003",
    },
    {
      error: {
        code: "P0001",
        message:
          "CONFLICT: Invoice was modified by another user during cancellation",
      },
      status: 409,
      code: "CONFLICT",
    },
  ...
  for (const testCase of cases) {
    ...
    const service = new InvoiceService(client as never);
    const error = await rejectedValue(() =>
      service.cancelInvoice(managerAuth, invoiceId, {
        cancel_reason: "Batch 9D-D atomic cancellation test",
      })
    );
    const response = errorResponse(error);
...
Deno.test("Batch 9D-D Receipt cancellation uses one governed RPC and returns only its committed result", async () => {
  ...
  assertEquals(
    (client.rpcCalls[0].params as MockRow).p_user_id,
    managerAuth.userId,
  );
  assertEquals(
    (client.rpcCalls[0].params as MockRow).p_company_id,
    managerAuth.companyId,
  );
  assertEquals(
    client.rpcCalls.some((call) =>
      call.functionName === "reverse_journal_entry"
  ...
  assertEquals(
    ...
    0,
    "Receipt cancellation must not perform separate table reads or writes",
  );
...
Deno.test("Batch 9D-D cheque clearance is one governed RPC with validated AuthContext parameters", async () => {
  ...
  assertEquals(
    client.operations.length,
    0,
    "Cheque clearance must not split journal and Receipt writes",
  );
  ...
  assert(
    body.includes(
      "callRpc<Receipt & { je_no?: string }>(this.client, 'clear_receipt_cheque'",
    ),
  );
  ...
  assert(
    !receiptIndex.includes(
      "String((body as Record<string, unknown>).clearance_date)",
    ),
  );
});
...
Deno.test("Batch 9D-D atomic Receipt operations preserve business, conflict, not-found, and internal error mapping", async () => {
  ...
  const cases = [
    {
      functionName: "cancel_receipt",
      message: "BR-RCT-CANCEL-ALLOC: active allocations exist",
      status: 400,
      code: "BR-RCT-CANCEL-ALLOC",
    },
    {
      functionName: "cancel_receipt",
    ...
    {
      functionName: "clear_receipt_cheque",
      message: "CONFLICT: Receipt was modified during clearance",
      status: 409,
      code: "CONFLICT",
    },
    {
  ...
  for (const testCase of cases) {
    const client = new MockSupabaseClient(tables, "service_role");
    client.rpcErrors[testCase.functionName] = {
      code: "P0001",
      message: testCase.message,
    };
    const service = new ReceiptService(client as never);
    const error = await rejectedValue(() =>
      testCase.functionName === "cancel_receipt"
        ? service.cancelReceipt(managerAuth, receiptId, {
          cancel_reason: "Atomic receipt cancellation mapping test",
        })
        : service.clearCheque(managerAuth, receiptId, {
          clearance_date: "2026-01-31",
        })
    );
    const response = errorResponse(error);
...
Deno.test("Batch 9D-D valid Debit Note reference remains separate from Linked Credit Note semantics", async () => {
  ...
  assertEquals(
    validated.cn_type,
    undefined,
    "Debit Note must not acquire Linked Credit Note semantics",
  );
  ...
  const createCall = client.rpcCalls.find((call) =>
    call.functionName === "fx_create_governed_invoice_draft"
  );
  assert(createCall, "Expected governed Debit Note creation RPC");
  const payload = (createCall.params as Record<string, unknown>)
    .p_invoice as Record<string, unknown>;
...
Deno.test("Batch 9D-D Debit Note creation fails closed for every invalid optional reference class", async () => {
  const cases: Array<{ label: string; reference: MockRow | null }> = [
    ...
    {
      label: "other company",
      reference: { company_id: "co-2", invoice_no: "SECRET-DN-OTHER-COMPANY" },
    },
    {
      label: "other customer",
      reference: {
        customer_id: "cust-b",
        invoice_no: "SECRET-DN-OTHER-CUSTOMER",
      },
    },
    { label: "currency mismatch", reference: { currency: "USD" } },
    { label: "Debit Note reference", reference: { doc_type: "Debit Note" } },
...
Deno.test("Batch 9D-D Invoice detail distinguishes successful empty lines from invoice_lines failure", async () => {
  ...
  const mutationClient = new MockSupabaseClient({}, "service_role");
  const failingReadClient = new MockSupabaseClient(
    detailTables,
    "authenticated",
  );
  ...
  assertEquals(
    (errorResponse(error).body.error as Record<string, unknown>).code,
    "INTERNAL_ERROR",
  );
  assertEquals(
    mutationClient.operations.length,
    0,
    "Invoice line failure must not retry through trusted client",
  );
});
...
Deno.test("Batch 9D-D CreditNoteService production composition delegates collection reads only to authenticated client", async () => {
  ...
  const result = await new CreditNoteService(
    mutationClient as never,
    readClient as never,
  )
  ...
  assertEquals(
    readClient.rpcCalls.filter((call) =>
      call.functionName === "ar_invoice_collection"
    ).length,
    1,
  );
  assertEquals(
    (readClient.rpcCalls.find((call) =>
      call.functionName === "ar_invoice_collection"
    )?.params as Record<string, unknown>).p_doc_type,
    "Credit Note",
  );
  assertEquals(
    mutationClient.rpcCalls.some((call) =>
      call.functionName === "ar_invoice_collection"
    ),
    false,
  );
  assertEquals(
    mutationClient.operations.some((op) =>
      op.table === "fx_booking_rate_decisions"
    ),
    false,
  );
});
...
Deno.test("Batch 9D-D CreditNoteService detail enrichment uses authenticated client only", async () => {
  ...
  assertEquals(
    readClient.operations.some((op) =>
      op.table === "fx_booking_rate_decisions" && op.op === "select"
    ),
    true,
  );
  assertEquals(
  ...
  assertEquals(
    mutationClient.operations.length,
    0,
    "Credit-note detail must not query through the trusted client",
  );
});
...
Deno.test("Batch 9D-D DebitNoteService detail enrichment uses authenticated client only", async () => {
  ...
  const result = await new DebitNoteService(
    mutationClient as never,
    readClient as never,
  )
  ...
  assertEquals(
    mutationClient.operations.length,
    0,
    "Debit-note detail must not query through the trusted client",
  );
});
...
Deno.test("Batch 9D-D authorized receipt detail succeeds for assigned Clerk and Finance Manager through read client only", async () => {
  ...
  const tables = {
    ...
    receipts: [{
      ...
      company_id: "co-1",
      customer_id: "cust-a",
      receipt_no: "RCT-DETAIL-001",
      status: "Draft",
      currency: "USD",
      base_amount: 13.5,
      fx_decision_id: "dec-receipt",
    }],
    fx_booking_rate_decisions: [{
      id: "dec-receipt",
      company_id: "co-1",
  ...
  for (const auth of [clerkAuth, managerAuth]) {
    const mutationClient = new MockSupabaseClient(tables, "service_role");
    const readClient = new MockSupabaseClient(tables, "authenticated");
    const result = await new ReceiptService(
      mutationClient as never,
      readClient as never,
    )
    ...
    assertEquals(
      readClient.operations.some((op) =>
        op.table === "receipts" && op.op === "select"
      ),
      true,
    );
    assertEquals(
    ...
    assertEquals(
      mutationClient.operations.length,
      0,
      "Receipt detail must not query through the trusted client",
    );
  }
});
...
Deno.test("Batch 9D-D RLS-hidden and nonexistent receipt detail share the same authorization-safe 404 contract", async () => {
  ...
  assert(
    hiddenError instanceof NotFoundError,
    "RLS-hidden receipt must map to NotFoundError",
  );
  assert(
    nonexistentError instanceof NotFoundError,
    "Nonexistent receipt must map to NotFoundError",
  );
  assertEquals(hiddenError.code, "NOT_FOUND");
  assertEquals(hiddenError.status, 404);
  assertJsonEquals(errorResponse(hiddenError), errorResponse(nonexistentError));
...
Deno.test("Batch 9D-D receipt detail preserves genuine PostgREST failures as HTTP 500", async () => {
  ...
  const mutationClient = new MockSupabaseClient({}, "service_role");
  const readClient = new MockSupabaseClient({}, "authenticated");
  readClient.queryErrors.receipts = {
    code: "XX000",
    message: "forced receipt database failure",
  };
  ...
  assert(
    !(error instanceof NotFoundError),
    "Unexpected receipt database failure must not become NotFoundError",
  );
  ...
  assertEquals(
    mutationClient.operations.length,
    0,
    "Failed receipt detail must not fall back to trusted client",
  );
});
...
Deno.test("Batch 9D-D unassigned receipt collection remains an empty authorized result", async () => {
  ...
  const result = await new ReceiptService(
    ...
    .listReceipts(clerkAuth, { customer_id: "cust-unassigned" }, {
      page: 1,
      page_size: 10,
    });
...
Deno.test("Batch 9D-D visible linked Credit Note reference uses authenticated caller context", async () => {
    ...
    invoices: [
      {
        ...
        company_id: "co-1",
        customer_id: "cust-a",
        invoice_no: "CN-LINK-001",
        doc_type: "Credit Note",
        cn_type: "Linked",
        ref_invoice_id: referenceId,
        status: "Open",
        currency: "MYR",
        base_total: 10,
        fx_decision_id: null,
  ...
  assertEquals(
    (result as Invoice & { ref_invoice?: Invoice }).ref_invoice?.id,
    referenceId,
  );
  assertEquals(
    readClient.operations.filter((op) =>
      op.table === "invoices" && op.op === "select"
    ).length,
    2,
  );
  assertEquals(
    mutationClient.operations.length,
    0,
    "Linked reference must not query trusted mutation client",
  );
});
...
Deno.test("Batch 9D-D caller-hidden and missing linked Credit Note references share authorization-safe 404", async () => {
  ...
  const hiddenMutationClient = new MockSupabaseClient({
    ...common,
    invoices: [root, reference],
  }, "service_role");
  const hiddenReadClient = new MockSupabaseClient({
    ...common,
    invoices: [root],
  }, "authenticated");
  const missingMutationClient = new MockSupabaseClient({
    ...common,
    invoices: [root],
  }, "service_role");
  const missingReadClient = new MockSupabaseClient({
    ...common,
    invoices: [root],
  }, "authenticated");
  ...
  const hiddenError = await rejectedValue(() =>
    new CreditNoteService(
      hiddenMutationClient as never,
      hiddenReadClient as never,
    )
      .getCreditNote(clerkAuth, creditNoteId)
  );
  const missingError = await rejectedValue(() =>
    new CreditNoteService(
      missingMutationClient as never,
      missingReadClient as never,
    )
      .getCreditNote(clerkAuth, creditNoteId)
  );
  ...
  assertJsonEquals(errorResponse(hiddenError), {
    ...
    body: {
      ...
      error: {
        code: "NOT_FOUND",
        message: `Credit Note not found: ${creditNoteId}`,
      },
    },
  });
  assertEquals(
    hiddenMutationClient.operations.length,
    0,
    "Caller-hidden reference must not query trusted client",
  );
  assertEquals(
    missingMutationClient.operations.length,
    0,
    "Missing reference must not query trusted client",
  );
  assert(
    !JSON.stringify(errorResponse(hiddenError)).includes(reference.invoice_no),
  );
});
...
Deno.test("Batch 9D-D genuine linked Credit Note reference query errors remain HTTP 500", async () => {
  ...
  assertEquals(
    mutationClient.operations.length,
    0,
    "Reference failure must not fall back to trusted client",
  );
});
...
Deno.test("Batch 9D-D missing authenticated read context fails before any delegated read query", async () => {
  ...
  const operations = [
    () =>
      new InvoiceService(mutationClient as never).listInvoices(clerkAuth, {}, {
        page: 1,
        page_size: 10,
      }),
    () =>
      new ReceiptService(mutationClient as never).getReceiptById(
        clerkAuth,
        "cccccccc-cccc-4ccc-8ccc-cccccccccccc",
      ),
    () =>
      new ReportService(mutationClient as never).getAgingSummary(
        managerAuth,
        "2026-01-31",
      ),
    () =>
      new CreditNoteService(mutationClient as never).listCreditNotes(
        clerkAuth,
        {},
        { page: 1, page_size: 10 },
      ),
    () =>
      new DebitNoteService(mutationClient as never).getDebitNote(
        clerkAuth,
  ...
  for (const operation of operations) {
    ...
    assert(
      message.includes("Authenticated read client is required"),
      `Expected fail-closed read-client error, got ${message}`,
    );
  }
  assertEquals(
    mutationClient.rpcCalls.length,
    0,
    "Missing read context must fail before migration-027 RPC invocation",
  );
  assertEquals(
    mutationClient.operations.length,
    0,
    "Missing read context must fail before SELECT invocation",
  );
});
...
Deno.test("Batch 9D-D invoice RPC summary includes 1,001 authorized rows while the returned page stays paginated", async () => {
  ...
  const client = new MockSupabaseClient({
    ...
    invoices: [
      ...authorizedRows,
      {
        ...
        id: "inv-unassigned",
        customer_id: "cust-unassigned",
        invoice_no: "CAP-UNASSIGNED",
      },
      {
        ...authorizedRows[0],
        id: "inv-hidden",
        customer_id: "cust-hidden",
        invoice_no: "CAP-HIDDEN",
      },
      {
        ...authorizedRows[0],
        id: "inv-other-company",
        company_id: "co-2",
        customer_id: "cust-other-company",
        invoice_no: "CAP-OTHER",
      },
    ],
    fx_booking_rate_decisions: [],
  });

  const result = await new InvoiceService(client as never, client as never)
    .listInvoices(
      clerkAuth,
  ...
  assertEquals(
    result.summary.current_balance_summary.by_currency[0].count,
    1001,
  );
  assertEquals(
    client.rpcCalls.filter((call) =>
  ...
  assertEquals(
    client.rpcCalls.some((call) =>
      call.functionName === "ar_invoice_collection_page"
    ),
    false,
  );
  assertEquals(
    client.rpcCalls.some((call) =>
      call.functionName === "ar_invoice_collection_summary"
    ),
    false,
  );
});
...
Deno.test("Batch 9D-D receipt RPC summary includes 1,001 authorized rows and excludes tenant/visibility adversaries", async () => {
  ...
  const client = new MockSupabaseClient({
    ...
    receipts: [
      ...authorizedRows,
      {
        ...
        id: "rct-unassigned",
        customer_id: "cust-unassigned",
        receipt_no: "CAP-UNASSIGNED",
      },
      {
        ...authorizedRows[0],
        id: "rct-hidden",
        customer_id: "cust-hidden",
        receipt_no: "CAP-HIDDEN",
      },
      {
        ...authorizedRows[0],
        id: "rct-other-company",
        company_id: "co-2",
        customer_id: "cust-other-company",
        receipt_no: "CAP-OTHER",
      },
    ],
    fx_booking_rate_decisions: [],
  });

  const result = await new ReceiptService(client as never, client as never)
    .listReceipts(
      clerkAuth,
  ...
  assertEquals(
    client.rpcCalls.some((call) =>
      call.functionName === "ar_receipt_collection_page"
    ),
    false,
  );
  assertEquals(
    client.rpcCalls.some((call) =>
      call.functionName === "ar_receipt_collection_summary"
    ),
    false,
  );
});
...
Deno.test("Gate A Invoice and Receipt pagination reaches page two and searches beyond the first 15 rows", async () => {
  ...
  assertEquals(
    invoiceSearch.invoices[0].invoice_no,
    "GATE-A-INVOICE-DEEP-SEARCH",
  );
  assertEquals(receiptSearch.total, 1);
  assertEquals(
    receiptSearch.receipts[0].receipt_no,
    "GATE-A-RECEIPT-DEEP-SEARCH",
  );
});
...
Deno.test("Batch 9D-D aging summary and by-customer contracts aggregate 1,001 rows inside RPCs before pagination", async () => {
  ...
  const byCustomer = await service.getAgingByCustomer(
    managerAuth,
    "2026-01-31",
    { page: 1, page_size: 1 },
  );
  ...
  assertEquals(
    client.rpcCalls.some((call) => call.functionName === "ar_aging_summary"),
    true,
  );
  assertEquals(
    client.rpcCalls.some((call) =>
...
Deno.test("Gate B credit-rating drill-down reconciles outstanding-only dashboard count and base amount over all pages", async () => {
  const customers = [
    ...
    },
    {
      id: "cust-hidden",
      company_id: "co-1",
      customer_id: "A-HIDDEN",
      customer_name: "Hidden Alpha",
      credit_limit: 5000,
      credit_rating: "A",
      is_deleted: false,
      is_hidden: true,
  ...
  assert(
    ratingParams.every((params) =>
      params.p_company_id === managerAuth.companyId
    ),
  );
  assert(
    ratingParams.every((params) => params.p_user_id === managerAuth.userId),
  );
});
...
Deno.test("Batch 9D-D ReportService.getCustomerStatement returns null mixed legacy scalars and base/per-currency balances", async () => {
  ...
  const statement = await new ReportService(client as never, client as never)
  ...
  assert(
    statement.lines.every((line) =>
      line.transaction_balance === null && line.balance === null
    ),
  );
});
...
Deno.test("Batch 9D-D customer statement uses stored booked-base snapshots when they diverge from rate recomputation", async () => {
  ...
  assertEquals(
    statement.meta.normalization_basis,
    "stored_booked_base_snapshot",
  );
  assert(
    statement.total_debit_base !== currentBaseFromBookedRate(100, 1.234567),
    "Stored invoice base_total must win over recomputation",
  );
});
...
Deno.test("Batch 9D-D customer statement RPC returns all 1,001 period movements in one authoritative contract", async () => {
  ...
  const client = new MockSupabaseClient({
    ...
    invoices: Array.from({ length: 1001 }, (_, index) => ({
      id: `stmt-${index}`,
      invoice_no: `STMT-${String(index).padStart(4, "0")}`,
      invoice_date: "2026-01-05",
      doc_type: "Invoice",
      total_amount: 1,
      base_total: 1,
  ...
  assertEquals(
    client.rpcCalls.some((call) =>
      call.functionName === "ar_customer_statement"
    ),
    true,
  );
});
...
Deno.test("Batch 9D-D mutation-only import/background composition returns committed results without elevated enrichment reads", async () => {
  const client = new MockSupabaseClient({
    ...
    invoices: [{
      ...
      outstanding: 0,
      status: "Draft",
      fx_decision_id: "missing-decision",
    }],
    invoice_lines: [],
    receipts: [{
      id: "rct-created",
  ...
  assertEquals(
    client.operations.some((op) => op.table === "fx_booking_rate_decisions"),
    false,
  );
  assertEquals(
    invoice.fx_decision,
    undefined,
    "Mutation-only response must omit optional decision enrichment",
  );
  assertEquals(
    receipt.fx_decision,
    undefined,
...
Deno.test("Batch 9D-D AllocationService retains a mutation-only nested ReceiptService without implicit read context", async () => {
  ...
  const nestedReceiptService =
    (allocationService as unknown as { receiptService: ReceiptService })
      .receiptService;
  ...
  const readError = await rejectedValue(() =>
    nestedReceiptService.getReceiptById(
      clerkAuth,
      "cccccccc-cccc-4ccc-8ccc-cccccccccccc",
    )
  );
  ...
  assertEquals(
    client.operations.length,
    operationsBeforeReadAttempt,
    "Missing read context must fail before SELECT",
  );
  assertEquals(
    client.rpcCalls.length,
    rpcCallsBeforeReadAttempt,
    "Missing read context must fail before RPC",
  );
});
...
Deno.test("Batch 9D-D authenticated post-create enrichment failure cannot mask a committed mutation", async () => {
  ...
  const invoice = await new InvoiceService(
    mutationClient as never,
    readClient as never,
  ).createInvoice(clerkAuth, {
    doc_type: "Invoice",
    customer_id: "cust-a",
    invoice_date: "2026-01-01",
    currency: "USD",
  ...
  const receipt = await new ReceiptService(
    mutationClient as never,
    readClient as never,
  ).createReceipt(clerkAuth, {
    customer_id: "cust-a",
    receipt_date: "2026-01-01",
    payment_method: "TT",
    currency: "USD",
...
Deno.test("Batch 9D-D allocation source preserves SQL-authoritative currency block and realized FX writer", async () => {
  const allocationSql = await read(
    "../../../../database/007_financial_rpcs.sql",
  );

  assert(
    allocationSql.includes("RAISE EXCEPTION 'BR-REC-003: Currency mismatch"),
    "Expected stable BR-REC-003 currency mismatch block",
  );
  assert(
    allocationSql.includes(
      "v_forex := ROUND(v_alloc_amt * (v_rct.exchange_rate - v_inv.exchange_rate), 2);",
    ),
    "Expected SQL-authoritative realized FX formula",
  );
  assert(
    allocationSql.includes("forex_gain_loss"),
    "Expected allocation_details.forex_gain_loss writer",
  );
});
...
Deno.test("Batch 9D-D migration 027 defines complete-set scoped aggregates and stored-snapshot statements", async () => {
  ...
  const importsService = await readImportServiceSource(import.meta.url);
  ...
  for (
    ...
    assert(
      ...
      `Expected migration 027 function ${functionName}`,
    );
  }
  assert(
    migration.includes("SECURITY INVOKER"),
    "Expected read RPCs to preserve invoker security",
  );
  assert(
    migration.includes("SET search_path = ''"),
    "Expected locked empty search_path",
  );
  assert(
    migration.includes("TO authenticated"),
    "Expected authenticated caller execution grants",
  );
  assert(
    migration.includes("FROM service_role"),
    "Expected ordinary read RPCs to reject service-role-only execution",
  );
  assert(
    migration.includes("auth.uid()"),
    "Expected database-derived authenticated identity",
  );
  assert(
    migration.includes("p_user_id IS DISTINCT FROM v_authenticated_user_id"),
    "Expected spoofed user parameter rejection",
  );
  assert(
    migration.includes("c.is_deleted = false") &&
      migration.includes("c.is_hidden = false"),
    "Expected hidden/deleted customer exclusion",
  );
  assert(
    migration.includes("public.user_customer_assignments"),
    "Expected AR Clerk assignment scope inside SQL",
  );
  assert(
    migration.includes("i.company_id = p_company_id") &&
      migration.includes("r.company_id = p_company_id"),
    "Expected company predicates before aggregation",
  );
  assert(
    migration.includes("SUM(f.outstanding_base)") &&
      migration.includes("SUM(f.unallocated_base)"),
    "Expected database-side current-base sums",
  );
  assert(
    migration.includes("SUM(f.base_total)") &&
      migration.includes("SUM(f.base_amount)"),
    "Expected stored original booked-base sums",
  );
  assert(
    migration.includes("'amount_basis', 'current_outstanding'") &&
      migration.includes("'amount_basis', 'current_unallocated'"),
    "Expected explicit summary amount bases",
  );
  assert(
    migration.includes("i.base_total AS base_debit") ||
      migration.includes("ELSE i.base_total END AS base_debit"),
    "Expected stored invoice snapshot statement debit",
  );
  assert(
    migration.includes("r.base_amount AS base_credit"),
    "Expected stored receipt snapshot statement credit",
  );
  assert(
    migration.includes("'stored_booked_base_snapshot'"),
    "Expected precise statement snapshot basis",
  );
  assert(!/\bEXECUTE\s+FORMAT\b/i.test(migration), "Dynamic SQL is prohibited");
  assert(
    !/\n(?:INSERT\s+INTO|UPDATE\s+public\.|DELETE\s+FROM)/i.test(migration),
    "Migration 027 must not mutate financial data",
  );
  assert(
    config.includes("max_rows = 1000"),
    "Expected documented PostgREST cap regression premise",
  );

  assert(
    invoiceService.includes("rpc('ar_invoice_collection'"),
    "Invoice service must consume one collection RPC",
  );
  assert(
    receiptService.includes("rpc('ar_receipt_collection'"),
    "Receipt service must consume one collection RPC",
  );
  for (
    const obsolete of [
      "ar_invoice_collection_summary",
      "ar_invoice_collection_page",
      "ar_receipt_collection_summary",
      "ar_receipt_collection_page",
    ]
  ) {
    assert(
      !migration.includes(`FUNCTION public.${obsolete}`),
      `Obsolete migration RPC must be absent: ${obsolete}`,
    );
    assert(
      !invoiceService.includes(`'${obsolete}'`) &&
        !receiptService.includes(`'${obsolete}'`),
      `Obsolete service RPC must be absent: ${obsolete}`,
    );
  }
  assert(
    invoiceIndex.includes("getUserClient(authorizationHeader)") &&
      invoiceIndex.includes(
        "dependencies.createService(req.headers.get('Authorization')!)",
      ),
  ...
  assert(
    reportIndex.includes("getUserClient(req.headers.get('Authorization')!)"),
    "Report index must inject the JWT-scoped migration-027 read client",
  );
  assert(
    creditNoteIndex.includes(
      "getUserClient(req.headers.get('Authorization')!)",
    ),
    "Credit-note index must inject the JWT-scoped delegated read client",
  );
  assert(
    debitNoteIndex.includes("getUserClient(req.headers.get('Authorization')!)"),
    "Debit-note index must inject the JWT-scoped delegated read client",
  );
  assert(
    invoiceService.includes("const readClient = this.requireReadClient();") &&
      invoiceService.includes("readClient.rpc('ar_invoice_collection'"),
    "Invoice collection RPC must require the caller-context read client",
  );
  assert(
    receiptService.includes("const readClient = this.requireReadClient();") &&
      receiptService.includes("readClient.rpc('ar_receipt_collection'"),
    "Receipt collection RPC must require the caller-context read client",
  );
  assert(
    reportService.includes("const readClient = this.requireReadClient();") &&
      reportService.includes("readClient.rpc(functionName, params)"),
    "Migration-027 report RPCs must require the caller-context read client",
  );
  for (const source of [invoiceService, receiptService, reportService]) {
    assert(
      !source.includes("readClient ?? client"),
      "Trusted clients must never be implicit read-client fallbacks",
    );
  }
  assert(
    creditNoteService.includes("new InvoiceService(this.client, readClient)"),
    "Credit-note delegated service must propagate authenticated read client",
  );
  assert(
    debitNoteService.includes("new InvoiceService(this.client, readClient)"),
    "Debit-note delegated service must propagate authenticated read client",
  );
  assert(
    importsService.includes("new InvoiceService(this.client)") &&
      importsService.includes("new ReceiptService(this.client)"),
    "Imports must retain explicit mutation-only nested services",
  );
  assert(
    !importsService.includes("this.invoiceService.listInvoices") &&
      !importsService.includes("this.invoiceService.getInvoiceById"),
    "Import mutation-only service must not invoke invoice user-domain reads",
  );
  assert(
    !importsService.includes("this.receiptService.listReceipts") &&
      !importsService.includes("this.receiptService.getReceiptById"),
    "Import mutation-only service must not invoke receipt user-domain reads",
  );
  assert(
    reportService.includes("const { data, error } = await this.client.rpc(") &&
      reportService.includes("'get_ar_dashboard_metrics'"),
    "The unchanged legacy dashboard RPC retains its established trusted client",
  );
  assert(
    !invoiceService.includes(
      "select('currency,outstanding,total_amount,exchange_rate,base_total')",
    ),
    "Invoice service must not aggregate capped raw summary rows",
  );
  assert(
    !receiptService.includes(
      "select('currency,unallocated_amount,receipt_amount,exchange_rate,base_amount')",
    ),
    "Receipt service must not aggregate capped raw summary rows",
  );
  assert(
    reportService.includes("'ar_aging_summary'"),
    "Aging summary must use SQL RPC",
  );
  assert(
    reportService.includes("'ar_aging_by_customer'"),
    "Aging by customer must use SQL RPC",
  );
  assert(
    reportService.includes("'ar_customer_statement'"),
    "Customer statement must use stored-snapshot SQL RPC",
  );
});
...
Deno.test("Batch 9D-D F-01/F-07 static SQL contract installs the consolidated lifecycle closure under quiescence", async () => {
  ...
  const invoiceCancelIndex = migration.indexOf(
    "CREATE FUNCTION public.cancel_invoice(",
  );
  const receiptCancelIndex = migration.indexOf(
    "CREATE FUNCTION public.cancel_receipt(",
  );
  const clearanceIndex = migration.indexOf(
  ...
  for (
    ...
    assert(
      migration.slice(lockIndex, linkedPreflightIndex).includes(table),
      `Migration lock must cover ${table}`,
    );
  }
  assert(migration.indexOf("SET LOCAL lock_timeout = '5s';") < lockIndex);
  assert(migration.includes("IN ACCESS EXCLUSIVE MODE;"));
  assertEquals(
    (migration.match(/LOCK TABLE/g) ?? []).length,
    1,
    "One deterministic barrier must acquire every governed table",
  );
  assert(
    !migration.includes("IN SHARE ROW EXCLUSIVE MODE;"),
    "No weaker lock may require a later Receipt lock upgrade",
  );
  assert(migration.includes("QUIESCENT INSTALLATION CONTRACT"));
  assert(
    migration.includes(
      "Confirm that no related financial transaction remains active",
    ),
  );
  assert(migration.includes("resume writes only after COMMIT succeeds"));
  assert(
    migration.indexOf("ALTER TABLE public.receipts") > lockIndex,
    "Strongest barrier must precede Receipt ALTER TABLE",
  );
  ...
  assert(
    migration.includes(
      "ref.status NOT IN ('Open', 'Overdue', 'Partially Paid')",
    ),
  );
  ...
  assert(
    migration.includes(
      "v_reference.doc_type NOT IN ('Invoice', 'Credit Note')",
    ),
  );
  assert(
    migration.includes(
      "v_reference.status NOT IN ('Open', 'Overdue', 'Partially Paid', 'Paid')",
    ),
  );
  ...
  assert(
    migration.includes(
      "NEW.status NOT IN ('Open', 'Overdue', 'Partially Paid', 'Paid')",
    ),
  );
  ...
  for (const functionName of triggerFunctions) {
    ...
    for (const role of ["PUBLIC", "anon", "authenticated", "service_role"]) {
      assert(
        migration.includes(
          `REVOKE ALL ON FUNCTION public.${functionName}() FROM ${role}`,
        ),
      );
    }
  }
...
Deno.test("Batch 9D-D Migration 028 uses one reference-row lock order without recursive financial mutation", async () => {
  ...
  const forwardStart = migration.indexOf(
    "CREATE OR REPLACE FUNCTION public.ar_validate_linked_credit_note_reference()",
  );
  const reverseStart = migration.indexOf(
  ...
  assert(
    forwardBody.includes("WHERE i.id = NEW.ref_invoice_id\n  FOR SHARE;"),
    "Forward writer must lock reference before validation",
  );
  ...
  assert(
    !/\bstatus\b/.test(forwardTrigger),
    "Posting-only CN status updates must not take a shared lock that is later upgraded",
  );
  assert(
    reverseBody.includes("UPDATE already owns the referenced Invoice row lock"),
    "Reverse writer must document the same reference-row serialization point",
  );
  assert(
    reverseBody.includes(
      "deliberately does not lock dependent Credit Note rows",
    ),
    "Reverse validation must avoid reference/CN reverse lock ordering",
  );
  assert(
    reverseBody.includes("SELECT EXISTS ("),
    "Reverse validation must inspect dependencies without mutating them",
  );
  assert(reverseBody.includes("v_invalid_debit_note_dependency_exists"));
  assert(
    reverseBody.includes(
      "BR-DN-REF: Debit Note reference is invalid or unavailable",
    ),
  );
  assert(
    !/\b(?:INSERT|UPDATE|DELETE)\s+(?:INTO\s+|FROM\s+)?public\.invoices\b/i
      .test(forwardBody + reverseBody),
    "Trigger functions must remain validation-only",
  );

  const postingMigration = await read(
    "../../../../database/023_fx_booking_rate_rpcs_and_immutability.sql",
  );
  assert(
    postingMigration.includes("WHEN v_new_os <= 0 THEN 'Paid'"),
    "Existing BR-CN-003 Paid transition must remain intact",
  );
  assert(
    postingMigration.includes(
      "WHEN v_new_os < v_ref.total_amount THEN 'Partially Paid'",
    ),
    "Existing BR-CN-003 partial transition must remain intact",
  );
  assert(
    postingMigration.includes(
      "SELECT * INTO v_ref FROM invoices WHERE id = v_inv.ref_invoice_id FOR UPDATE;",
    ),
    "Posting must retain its authoritative reference lock",
  );
});
...
Deno.test("Batch 9D-D Draft Invoice header edits keep FX and non-FX changes in one governed transaction", async () => {
  ...
  const reverseAllocationStart = migration.indexOf(
  ...
  assertEquals(
    (migration.match(/CREATE FUNCTION public\.update_draft_invoice\(/g) ?? [])
      .length,
    1,
  );
  ...
  const fxUpdate = body.indexOf(
    "PERFORM public.fx_update_governed_invoice_fx(",
  );
  const headerUpdate = body.indexOf("UPDATE public.invoices i");
  assert(
    parentLock >= 0 && parentForUpdate > parentLock &&
      fxUpdate > parentForUpdate && headerUpdate > fxUpdate,
  );
...
Deno.test("Batch 9D-D F-05 posted reference_no correction uses one narrow governed and audited path", async () => {
  ...
  const corrected = await new InvoiceService(client as never)
    .correctPostedReference(
      clerkAuth,
      invoiceId,
      "PO-NEW",
    );
  ...
  assertEquals(
    client.operations.length,
    0,
    "Posted reference correction must not compose trusted table DML",
  );
  ...
  const invoicePrd = await read(
    "../../../../PRD_Part2_Invoicing_Credit_Notes.md",
  );
  assert(
    invoicePrd.includes(
  ...
  assertEquals(
    (migration.match(
      /CREATE FUNCTION public\.correct_posted_invoice_reference\(/g,
    ) ?? []).length,
    1,
  );
  assert(body.includes("SECURITY DEFINER"));
  ...
  const auditInsert = body.indexOf(
    "INSERT INTO public.credit_control_logs",
    referenceUpdate,
  );
  assert(
    parentLock >= 0 && forUpdate > parentLock && referenceUpdate > forUpdate &&
      auditInsert > referenceUpdate,
  );
  assert(
    body.includes(
      "v_invoice.doc_type NOT IN ('Invoice', 'Debit Note', 'Credit Note')",
    ),
  );
  ...
  for (
    ...
    assert(
      ...
      `Reference correction must not mutate ${forbidden}`,
    );
  }
  ...
  const postedClause = lifecycle.slice(
    lifecycle.indexOf("-- BR-INV-001"),
    lifecycle.indexOf("v_system_change :="),
  );
  assert(
    lifecycle.includes("OR NEW.reference_no IS DISTINCT FROM OLD.reference_no"),
  );
  assert(
    !terminalClause.includes(
      "NEW.reference_no IS DISTINCT FROM OLD.reference_no",
    ),
  );
  assert(
    !postedClause.includes(
      "NEW.reference_no IS DISTINCT FROM OLD.reference_no",
    ),
  );
  assert(
  ...
  assert(
    lifecycle.includes(
      "BR-DOC-AUTHORITY: Financial structure requires a governed mutation",
    ),
  );
  ...
  for (const role of ["PUBLIC", "anon", "authenticated", "service_role"]) {
    assert(
      migration.includes(`REVOKE ALL ON FUNCTION ${signature} FROM ${role};`),
    );
  }
});
  ...
  return createReceiptHandler({
    authenticate: (req, companyId) => {
      assertEquals(
        ...
        "Bearer receipt-route-test-token",
      );
...
Deno.test("Gate A PATCH Receipt route reaches the governed Draft FX update with authenticated tenant authority", async () => {
  ...
  const corsResponse = await receiptFxHandler(client)(
    new Request(
      `https://example.test/functions/v1/receipts/${receiptFxRouteReceiptId}`,
      {
        method: "OPTIONS",
  ...
  assert(
    corsResponse.headers.get("Access-Control-Allow-Methods")?.split(",").map((
      method,
    ) => method.trim())
      .includes("PATCH"),
...
Deno.test("Gate A PATCH Receipt route rejects mixed authority, cross-company, unauthorized, and non-Draft requests before RPC", async () => {
  for (
    ...
    const client = new MockSupabaseClient(
      receiptFxRouteTables(),
      "service_role",
    );
    const response = await receiptFxHandler(client)(
      receiptFxRequest(invalidBody),
    );
  ...
  const crossCompanyResponse = await receiptFxHandler(crossCompanyClient)(
    receiptFxRequest({
      currency: "SGD",
      fx_reference_rate_id: receiptFxRouteReferenceId,
    }),
...
Deno.test("Batch 9D-D F-05/F-07 production handler reaches posted reference RPC with AuthContext-bound identity", async () => {
  ...
  assertEquals(
    client.rpcCalls[1].functionName,
...
Deno.test("Batch 9D-D F-07 posted reference route rejects payload authority, invalid values, and unsupported methods", async () => {
  ...
  for (const invalidCase of invalidCases) {
    ...
    const response = await postedReferenceHandler(client)(
      postedReferenceRequest(
        `/${postedReferenceRouteInvoiceId}/reference`,
        "PATCH",
        invalidCase.body,
      ),
    ...
    assertEquals(
      response.status,
      400,
      `${invalidCase.name} must return HTTP 400`,
    );
    const responseBody = await responseJson(response);
    assertEquals(
    ...
    assertEquals(
      client.rpcCalls.length,
      0,
      `${invalidCase.name} must not reach an RPC`,
    );
    const serialized = JSON.stringify(responseBody);
    assert(
      !serialized.includes("attacker-user") &&
        !serialized.includes("attacker-company"),
    );
  }

  const methodClient = new MockSupabaseClient({}, "service_role");
  const methodResponse = await postedReferenceHandler(methodClient)(
    postedReferenceRequest(
      `/${postedReferenceRouteInvoiceId}/reference`,
      "POST",
    ),
  ...
  assertEquals(
    (await responseJson(methodResponse)).error instanceof Object,
    true,
  );
  assertEquals(methodClient.rpcCalls.length, 0);

  const corsResponse = await postedReferenceHandler(methodClient)(
    postedReferenceRequest(
      `/${postedReferenceRouteInvoiceId}/reference`,
      "OPTIONS",
    ),
...
Deno.test("Batch 9D-D F-07 generic Invoice PATCH remains Draft-only update composition", async () => {
  const calls: Array<
    { method: string; auth: AuthContext; id: string; input: CreateInvoiceInput }
  > = [];
  const service = {
    updateDraftInvoice: (
      auth: AuthContext,
...
Deno.test("Batch 9D-D F-07 posted reference route preserves governed RPC error mapping and disclosure", async () => {
  const cases = [
    {
      message: "BR-DOC-REFERENCE: correction rejected",
      status: 400,
      code: "BR-DOC-REFERENCE",
    },
    ...
    {
      message: "column private_financial_detail does not exist",
      status: 500,
      code: "INTERNAL_ERROR",
    },
  ];
...
Deno.test("Batch 9D-D Migration 028 makes every Invoice-family lifecycle classification-independently terminal", async () => {
  ...
  const receiptFunctionStart = migration.indexOf(
    "CREATE OR REPLACE FUNCTION public.ar_enforce_receipt_lifecycle()",
  );
  const forwardTriggerStart = migration.indexOf(
    "CREATE TRIGGER trg_ar_linked_credit_note_reference_forward",
  );
  const reverseTriggerStart = migration.indexOf(
    "CREATE TRIGGER trg_ar_linked_credit_note_reference_reverse",
  );
  const terminalTriggerStart = migration.indexOf(
    "CREATE TRIGGER trg_ar_financial_document_lifecycle",
  );
  const receiptTriggerStart = migration.indexOf(
  ...
  const terminalTrigger = migration.slice(
    terminalTriggerStart,
    receiptTriggerStart,
  );
  ...
  assertEquals(
    (migration.match(
      /CREATE OR REPLACE FUNCTION public\.ar_prevent_cancelled_document_mutation\(\)/g,
    ) ?? []).length,
    1,
  );
  assertEquals(
    (migration.match(/CREATE TRIGGER trg_ar_financial_document_lifecycle/g) ??
      []).length,
    1,
  );
  ...
  assert(
    !terminalClause.includes("OLD.doc_type = 'Credit Note'"),
    "Terminality must not depend on mutable document classification",
  );
  assert(
    !terminalClause.includes("OLD.cn_type = 'Linked'"),
    "Terminality must not depend on mutable CN classification",
  );
  for (
    const field of [
      "id",
  ...
  assert(
    !terminalBody.includes("FOR SHARE"),
    "Terminal rejection must not enter the reference-row posting lock order",
  );
  assert(
    !/\bINSERT\s+INTO\b/i.test(terminalBody),
    "Lifecycle trigger must not create financial records",
  );
  assert(
    !/\bUPDATE\s+public\./i.test(terminalBody),
    "Lifecycle trigger must not mutate another financial row",
  );
  assert(
    !/\bDELETE\s+FROM\b/i.test(terminalBody),
    "Lifecycle trigger must not delete financial records",
  );
  ...
  assert(
    !/\bstatus\b/.test(forwardTrigger),
    "Ordinary posting status transitions must remain outside forward FOR SHARE validation",
  );
  assert(
    !migration.includes("ar_prevent_cancelled_linked_credit_note_reactivation"),
  );
  assert(!migration.includes("trg_ar_cancelled_linked_credit_note_terminal"));
  assert(
    !migration.includes("CREATE TRIGGER trg_ar_cancelled_document_terminal"),
  );
...
Deno.test("Batch 9D-D Cancelled-document transition model covers every protected field and valid lifecycle transitions", async () => {
  ...
  for (const targetStatus of ["Draft", "Open", "Paid", null, "Written Off"]) {
    assertEquals(
      isRejectedTransition(cancelledInvoice, {
        ...cancelledInvoice,
        status: targetStatus,
      }),
      true,
  ...
  const cancelledDebitNote: TerminalRow = {
    ...cancelledInvoice,
    doc_type: "Debit Note",
  };
  assertEquals(
    isRejectedTransition(cancelledStandalone, {
      ...cancelledStandalone,
      status: "Open",
    }),
    true,
  ...
  assertEquals(
    isRejectedTransition(cancelledDebitNote, {
      ...cancelledDebitNote,
      status: "Open",
    }),
    true,
  ...
  assertEquals(
    isRejectedTransition(cancelledInvoice, { ...cancelledInvoice }),
    false,
    "Cancelled no-op must remain harmless",
  );

  const draftInvoice = { ...cancelledInvoice, status: "Draft" };
  const openInvoice = { ...cancelledInvoice, status: "Open" };
  assertEquals(
    isRejectedTransition(draftInvoice, { ...draftInvoice, status: "Open" }),
    false,
  );
  assertEquals(
    isRejectedTransition(draftInvoice, { ...draftInvoice, status: "Paid" }),
    false,
  );
  assertEquals(
    isRejectedTransition(draftInvoice, {
      ...draftInvoice,
      status: "Cancelled",
    }),
    false,
  ...
  assertEquals(
    isRejectedTransition(openInvoice, { ...openInvoice, status: "Paid" }),
    false,
  );
  assertEquals(
    isRejectedTransition(openInvoice, { ...openInvoice, status: "Cancelled" }),
    false,
  );
...
Deno.test("Batch 9D-D Migration 028 cancellation RPC keeps checks, reversal, and status update atomic", async () => {
  ...
  assertEquals(
    (migration.match(/CREATE FUNCTION public\.cancel_invoice\(/g) ?? []).length,
    1,
    "Expected one non-overloaded cancellation function",
  );
  assert(body.includes("RETURNS JSONB"));
  assert(body.includes("VOLATILE"));
  assert(body.includes("SECURITY DEFINER"));
  ...
  assert(
    body.includes(
      "BR-DN-REF: Debit Note reference integrity prevents this document cancellation",
    ),
  );
  ...
  assert(
    body.includes(
      "v_invoice.outstanding IS DISTINCT FROM v_invoice.total_amount",
    ),
  );
  assert(body.includes("IF v_primary_je_count <> 1 THEN"));
  assert(body.includes("AND version = p_expected_version"));
  assert(
    !/\bCOMMIT\b/i.test(body),
    "Cancellation function must not escape its caller transaction",
  );
  assert(
    !/EXECUTE\s+FORMAT/i.test(body),
    "Cancellation function must not use dynamic SQL",
  );
  ...
  const serviceEnd = invoiceService.indexOf(
    "// GET / LIST INVOICES",
    serviceStart,
  );
  const serviceBody = invoiceService.slice(serviceStart, serviceEnd);
  assert(
    serviceBody.includes("callRpc<Invoice>(this.client, 'cancel_invoice'"),
  );
...
Deno.test("Batch 9D-D Migration 028 protects lifecycle headers, lines, allocations, and journal evidence", async () => {
  ...
  const journalStart = migration.indexOf(
    "CREATE OR REPLACE FUNCTION public.ar_protect_journal_entry()",
  );
  const journalLineStart = migration.indexOf(
  ...
  assert(
    invoiceBody.includes(
      "current_setting('app.ar_draft_delete', TRUE) IS DISTINCT FROM 'on'",
    ),
  );
  assert(
    invoiceBody.includes(
      "BR-DOC-AUTHORITY: Draft deletion requires the governed deletion operation",
    ),
  );
  ...
  assert(
    invoiceBody.includes("ROUND(NEW.total_amount * NEW.exchange_rate, 2)"),
  );
  assert(
    invoiceBody.includes("NEW.reference_no IS DISTINCT FROM OLD.reference_no"),
  );
  ...
  assert(
    receiptBody.includes(
      "BR-RCT-AUTHORITY: Draft deletion requires the governed deletion operation",
    ),
  );
  assert(receiptBody.includes("NEW.status NOT IN ('Cancelled', 'Bounced')"));
  assert(
    receiptBody.includes(
      "ROUND(NEW.allocated_amount + NEW.unallocated_amount, 2)",
    ),
  );
  assert(
    receiptBody.includes("ROUND(NEW.receipt_amount * NEW.exchange_rate, 2)"),
  );
  ...
  assert(
    allocationBody.includes("OLD.status <> 'Active'") &&
      allocationBody.includes("NEW.status <> 'Reversed'"),
  );
  assert(
    cnAllocationBody.includes("OLD.status <> 'Active'") &&
      cnAllocationBody.includes("NEW.status <> 'Reversed'"),
  );
  assert(
    allocationBody.includes("NEW.receipt_rate IS DISTINCT FROM v_receipt_rate"),
  );
  assert(
    allocationBody.includes("NEW.invoice_rate IS DISTINCT FROM v_invoice_rate"),
  );
  assert(
    allocationBody.includes("ROUND(NEW.allocated_amount * v_receipt_rate, 2)"),
  );
  assert(
    allocationBody.includes("NEW.forex_gain_loss IS DISTINCT FROM ROUND("),
  );
  assert(
    allocationBody.includes(
      "NEW.allocated_amount + NEW.discount_amount > v_invoice_outstanding + 0.01",
    ),
  );
  ...
  assert(
    cnAllocationBody.includes(
      "v_cn_status NOT IN ('Open', 'Partially Paid', 'Paid')",
    ),
  );
  assert(
    cnAllocationBody.includes(
      "v_invoice_status NOT IN ('Open', 'Overdue', 'Partially Paid', 'Paid')",
    ),
  );
  ...
  assert(
    cnAllocationBody.includes("PERFORM public.rpc_check_customer_access("),
  );
  ...
  assert(
    !journalLineBody.includes("ga.is_active = TRUE"),
    "Reversals must retain historical GL accounts even after deactivation",
  );
  assert(journalLineBody.includes("BR-JE-LINE-PARENT"));
  assert(
    migration.includes("CREATE UNIQUE INDEX uq_journal_entries_one_reversal"),
  );
  ...
  for (
    ...
    assertEquals(
      (migration.match(new RegExp(`CREATE TRIGGER ${trigger}`, "g")) ?? [])
        .length,
      1,
    );
  }
...
Deno.test("Batch 9D-D FX booking provenance rejects routine direct DML and binds new decision actors", async () => {
  ...
  assert(
    decisionStart >= 0 && eventStart > decisionStart &&
      triggerStart > eventStart,
  );
  ...
  assert(
    eventBody.includes("IF TG_OP <> 'INSERT' OR NOT v_owner_authorized THEN"),
  );
  assert(
    migration.includes("CREATE TRIGGER trg_ar_fx_booking_decision_integrity"),
  );
  assert(
    migration.includes(
...
Deno.test("Batch 9D-D Invoice line transition and stored-amount model matches the authoritative trigger contract", () => {
  ...
  const mayMutateLine = (parentStatus: string, _operation: LineOperation) =>
    parentStatus === "Draft";
  ...
  for (const operation of ["INSERT", "UPDATE", "DELETE"] as const) {
    assertEquals(
      mayMutateLine("Draft", operation),
      true,
      `Draft ${operation} must remain available`,
    );
    for (
      const status of [
        "Open",
  ...
  assert(
    String(excessiveDiscount).includes(
      "Discount amount cannot exceed the gross line amount",
    ),
  );
});
...
Deno.test("Batch 9D-D F-03 direct PostgREST lifecycle errors preserve safe business mapping and disclosure", async () => {
  const cases = [
    {
      message:
        "BR-LINE-IMMUTABLE: Invoice lines may change only while the parent is Draft",
      status: 400,
      code: "BR-LINE-IMMUTABLE",
    },
    {
    ...
    {
      message: "CONFIG: Bank account configuration is unavailable",
      status: 400,
      code: "CONFIG",
    },
    {
    ...
    {
      message: "NOT_FOUND: Parent financial document is unavailable",
      status: 404,
      code: "NOT_FOUND",
    },
  ];
  ...
  try {
    ...
    try {
      throwDatabaseError(
        {
          message:
            "XX000: forced storage failure containing AUTH: but without a governed prefix",
        },
        "Failed lifecycle mutation",
      );
    } catch (error) {
  ...
  assert(
    !overdueHandler.includes(
      "result.errors.push(error instanceof Error ? error.message",
    ),
  );
  assert(
    /result\.errors\.push\(["']Daily overdue task failed["']\)/.test(
      overdueHandler,
    ),
  );
  assert(
    /console\.error\(["']\[DAILY-OVERDUE\] Fatal error:["'], error\)/.test(
      overdueHandler,
    ),
  );
  ...
  assert(
    invoiceService.includes(
      "callRpc<InvoiceLine[]>(this.client, 'add_draft_invoice_lines'",
    ),
  );
  assert(
    invoiceService.includes(
      "callRpc<InvoiceLine>(this.client, 'update_draft_invoice_line'",
    ),
  );
  assert(
    invoiceService.includes("callRpc(this.client, 'delete_draft_invoice_line'"),
  );
});
...
Deno.test("Batch 9D-D Draft Invoice line edits are parent-locked one-RPC aggregate mutations", async () => {
  ...
  const service = await readInvoiceServiceSource(import.meta.url);
  ...
  assert(
    addStart >= 0 && updateStart > addStart && deleteStart > updateStart &&
      cancelStart > deleteStart,
  );
...
Deno.test("Batch 9D-D Cancelled header transition model covers every financial and audit evidence field", async () => {
  ...
  const oldRow = Object.fromEntries(
    fields.map((field) => [field, `${field}-old`]),
  ) as Record<string, unknown>;
  oldRow.status = "Cancelled";
  const rejected = (next: Record<string, unknown>) =>
    oldRow.status === "Cancelled" &&
  ...
  for (const field of fields) {
    ...
    assertEquals(
      rejected({ ...oldRow, [field]: null }),
      true,
      `Cancelled ${field} NULL mutation must fail`,
    );
    assertEquals(
      rejected({ ...oldRow, [field]: `${field}-new` }),
      true,
      `Cancelled ${field} mutation must fail`,
    );
  }
  assertEquals(
    rejected({ ...oldRow }),
    false,
    "Protected-field no-op remains harmless",
  );
  assert(
    !fields.includes("updated_at"),
    "Automatic updated_at is intentionally outside the terminal evidence comparison",
  );
  assert(
    !fields.includes("internal_remarks") && !fields.includes("invoice_remarks"),
    "Narrative remarks remain PRD-approved post-state edits",
  );
  assert(
    !fields.includes("reference_no"),
    "reference_no changes are permitted only through the governed correction RPC",
  );
});
...
Deno.test("Batch 9D-D Migration 028 makes Receipt cancellation and cheque clearance fully atomic", async () => {
  ...
  const clearanceStart = migration.indexOf(
    "CREATE FUNCTION public.clear_receipt_cheque(",
  );
  const revokeStart = migration.indexOf(
    "REVOKE ALL ON FUNCTION public.ar_validate_linked_credit_note_reference()",
    clearanceStart,
  );
  ...
  assertEquals(
    (migration.match(/CREATE FUNCTION public\.cancel_receipt\(/g) ?? []).length,
    1,
  );
  assertEquals(
    (migration.match(/CREATE FUNCTION public\.clear_receipt_cheque\(/g) ?? [])
      .length,
    1,
  );
  ...
  assert(
    cancellation.includes(
      "v_receipt.unallocated_amount IS DISTINCT FROM v_receipt.receipt_amount",
    ),
  );
  ...
  const postingJournalLock = clearance.indexOf(
    "INTO v_original_je_id",
    clearForUpdate,
  );
  const postingJournalForUpdate = clearance.indexOf(
    "FOR UPDATE;",
    postingJournalLock,
  );
  const clearJournalInsert = clearance.indexOf(
    "INSERT INTO public.journal_entries",
  );
  const clearLineInsert = clearance.indexOf(
    "INSERT INTO public.journal_entry_lines",
  );
  const valueDateUpdate = clearance.indexOf("UPDATE public.receipts");
  assert(
    clearLock >= 0 && clearForUpdate > clearLock &&
      postingJournalLock > clearForUpdate &&
      postingJournalForUpdate > postingJournalLock &&
  ...
  for (
    const [signature, role] of [
      ["public.cancel_receipt(UUID, UUID, UUID, TEXT)", "service_role"],
      ["public.clear_receipt_cheque(UUID, UUID, UUID, DATE)", "service_role"],
    ]
  ) {
    assert(
      migration.includes(`GRANT EXECUTE ON FUNCTION ${signature} TO ${role}`),
    );
    for (
      const revokedRole of ["PUBLIC", "anon", "authenticated", "service_role"]
    ) {
      assert(
        migration.includes(
          `REVOKE ALL ON FUNCTION ${signature} FROM ${revokedRole}`,
        ),
      );
    }
  }
...
Deno.test("Batch 9D-D F-06 consolidated guards preserve posting, constrained overdue, and one-RPC Draft deletion", async () => {
  ...
  assert(
    journalBody.includes(
      "NEW.total_debit IS NOT DISTINCT FROM OLD.total_debit",
    ),
  );
  assert(
    journalBody.includes(
      "NEW.total_credit IS NOT DISTINCT FROM OLD.total_credit",
    ),
  );
  ...
  const invoiceLifecycle = migration.slice(
    invoiceLifecycleStart,
    receiptLifecycleStart,
  );
  assert(
    invoiceLifecycle.includes(
      "v_direct_overdue_transition := OLD.doc_type IN ('Invoice', 'Debit Note')",
    ),
  );
  assert(
    invoiceLifecycle.includes("AND OLD.status IN ('Open', 'Partially Paid')"),
  );
  assert(invoiceLifecycle.includes("OLD.due_date < CURRENT_DATE"));
  assert(invoiceLifecycle.includes("OLD.outstanding > 0"));
  assert(
    invoiceLifecycle.includes(
      "OLD.doc_type = 'Credit Note' AND NEW.status = 'Overdue'",
    ),
  );
  assert(
    invoiceLifecycle.includes(
      "BR-CN-004: Credit Notes cannot enter the Overdue lifecycle",
    ),
  );
  ...
  assertEquals(
    directOverdueAllowed("Debit Note", "Partially Paid", true, 10),
    true,
  );
  assertEquals(
    directOverdueAllowed("Credit Note", "Open", true, 10),
    false,
    "Standalone CN must not become Overdue",
  );
  assertEquals(
    directOverdueAllowed("Credit Note", "Partially Paid", true, 10),
    false,
    "Linked/CN balance state must not become Overdue",
  );
...
Deno.test("Batch 9D-D F-04 Invoice and Receipt Draft deletion restore scoped flags around one governed RPC", async () => {
  ...
  for (
    ...
    const captureDraftFlag = body.indexOf(
      "current_setting('app.ar_draft_delete', TRUE)",
    );
    const captureFxFlag = body.indexOf(
      "current_setting('app.fx_governed_mutation', TRUE)",
    );
    const successRestore = body.indexOf(
      "COALESCE(NULLIF(v_prior_draft_delete, ''), 'off')",
      headerDelete,
    );
    const successReturn = body.indexOf(
      "RETURN jsonb_build_object('deleted'",
      successRestore,
    );
    const exceptionBlock = body.indexOf(
      "EXCEPTION\n  WHEN OTHERS THEN",
      successReturn,
    );
    const exceptionRestore = body.indexOf(
      "COALESCE(NULLIF(v_prior_draft_delete, ''), 'off')",
      exceptionBlock,
    );
    assert(
      start >= 0 && parent >= 0 && parentLock > parent &&
        importCheck > parentLock &&
        eventDelete > importCheck && pointer > eventDelete &&
        decisionDelete > pointer && headerDelete > decisionDelete,
    );
    assert(
      captureDraftFlag >= 0 && captureFxFlag > captureDraftFlag &&
        captureFxFlag < eventDelete,
    );
    assert(
      successRestore > headerDelete && successReturn > successRestore &&
        exceptionBlock > successReturn && exceptionRestore > exceptionBlock,
    );
    assert(
      body.includes("pg_catalog.set_config('app.ar_draft_delete', 'on', TRUE)"),
    );
    assert(
      body.includes(
        "pg_catalog.set_config('app.fx_governed_mutation', 'on', TRUE)",
      ),
    );
    assertEquals(
      (body.match(/COALESCE\(NULLIF\(v_prior_draft_delete, ''\), 'off'\)/g) ??
        []).length,
      2,
    ...
    assertEquals(
      (body.match(
        /COALESCE\(NULLIF\(v_prior_fx_governed_mutation, ''\), 'off'\)/g,
      ) ?? []).length,
      2,
    ...
    assert(
      migration.includes(
        `GRANT EXECUTE ON FUNCTION ${contract.signature} TO service_role;`,
      ),
    );
    for (const role of ["PUBLIC", "anon", "authenticated", "service_role"]) {
      assert(
        migration.includes(
          `REVOKE ALL ON FUNCTION ${contract.signature} FROM ${role};`,
        ),
      );
    }
  }

  const restoredFlag = (prior: string | null) =>
    prior && prior.length > 0 ? prior : "off";
  assertEquals(
    restoredFlag(null),
    "off",
    "An unset outer flag restores to a safe disabled value",
  );
  assertEquals(
    restoredFlag("off"),
    "off",
    "A disabled outer flag stays disabled after success or failure",
  );
  assertEquals(
    restoredFlag("on"),
    "on",
    "A nested governed invocation restores its caller-owned enabled scope",
  );
  ...
  assert(
    appendOnlyBody.includes(
      "current_setting('app.ar_draft_delete', TRUE) = 'on'",
    ),
  );
  assert(appendOnlyBody.includes("i.status = 'Draft'"));
  assert(appendOnlyBody.includes("r.status = 'Draft'"));
  assert(
    appendOnlyBody.includes("Booking-rate decision events are append-only"),
  );
  for (const role of ["PUBLIC", "anon", "authenticated", "service_role"]) {
    assert(migration.includes(
  ...
  assert(
    receiptIndex.includes("await service.deleteDraftReceipt(auth, params.id)"),
  );
});
...
Deno.test("Batch 9D-D allocation reversal uses the same parent-first lock order as bounce and cancellation", async () => {
  ...
  const invoiceCancelStart = migration.indexOf(
    "CREATE FUNCTION public.cancel_invoice(",
    reverseStart,
  );
  ...
  const allocationMutation = body.indexOf(
    "UPDATE public.allocation_details",
    invoiceForUpdate,
  );
  const invoiceMutation = body.indexOf(
    "UPDATE public.invoices",
    allocationMutation,
  );
  const receiptMutation = body.indexOf(
    "UPDATE public.receipts",
    invoiceMutation,
  );
  const journalLock = body.indexOf(
...
Deno.test("Batch 9D-D F-02 static SQL contract keeps posted Credit Notes irreversible across preflight and reversal", async () => {
  ...
  const legacyFinancialRpcs = await read(
    "../../../../database/007_financial_rpcs.sql",
  );
  const preflightStart = migration.indexOf(
    "-- Migration 027 and earlier allocation logic did not exclude Credit Notes",
  );
  const preflightEnd = migration.indexOf(
    "-- Reversal creation already locks the original journal entry",
    preflightStart,
  );
  ...
  assert(
    !/\b(?:UPDATE|DELETE)\s+public\./i.test(preflight),
    "Preflight must never rewrite historical allocation evidence",
  );
  ...
  const creditNoteGuard = reverseBody.indexOf(
    "IF v_inv.doc_type = 'Credit Note'",
    targetLock,
  );
  const allowedTypeGuard = reverseBody.indexOf(
    "v_inv.doc_type NOT IN ('Invoice', 'Debit Note')",
    creditNoteGuard,
  );
  const allocationMutation = reverseBody.indexOf(
    "UPDATE public.allocation_details",
    allowedTypeGuard,
  );
  assert(
    targetLock >= 0 && creditNoteGuard > targetLock &&
      allowedTypeGuard > creditNoteGuard &&
      allocationMutation > allowedTypeGuard,
  );
  assert(
    reverseBody.includes(
      "BR-CN-004: Receipt allocation reversal cannot reopen a posted Credit Note",
    ),
  );
  ...
  const lifecycle = migration.slice(lifecycleStart, receiptStart);
  assert(
    lifecycle.includes("OLD.doc_type = 'Credit Note' AND OLD.status = 'Paid'"),
  );
  assert(
    lifecycle.includes(
      "BR-CN-004: Posted Credit Notes are irreversible; issue a Debit Note instead",
    ),
  );
  assert(
    lifecycle.includes(
  ...
  const supportsReceiptReversal = (docType: string) =>
    ["Invoice", "Debit Note"].includes(docType);
...
Deno.test("Batch 9D-D initial and Draft structural boundaries enforce visible same-tenant parents", async () => {
  ...
  for (const body of [invoiceBody, receiptBody]) {
    assert(
      body.includes("current_user = pg_catalog.pg_get_userbyid(c.relowner)"),
    );
    ...
    assert(
      body.indexOf("IF TG_OP = 'DELETE' THEN") <
        body.indexOf("v_structural_change :="),
    );
  }
  assert(
    invoiceBody.includes(
  ...
  assert(
    receiptBody.includes(
      "BR-RCT-AUTHORITY: Receipt structure requires a governed mutation",
    ),
  );
  ...
  assert(
    allocationBody.includes(
      "v_invoice_doc_type NOT IN ('Invoice', 'Debit Note')",
    ),
  ...
  assert(
    customerScopeBody.includes("MESSAGE = 'NOT_FOUND: Customer not found'"),
  );
  assert(
    customerScopeBody.includes(
      "ur.role IN ('AR Supervisor', 'Finance Manager')",
    ),
  );
...
Deno.test("Batch 9D-D production constructor composition keeps trusted and authenticated clients distinct", async () => {
  ...
  for (const indexSource of [reportIndex, creditIndex, debitIndex]) {
    assert(
      indexSource.includes("getUserClient(req.headers.get('Authorization')!)"),
      "Every user-request read composition must inject the JWT-scoped client",
    );
  }
  assert(
    creditService.includes("new InvoiceService(this.client, readClient)"),
    "Credit-note nested invoice service must receive read client explicitly",
  );
  assert(
    debitService.includes("new InvoiceService(this.client, readClient)"),
    "Debit-note nested invoice service must receive read client explicitly",
  );
  assert(
    importService.includes("new InvoiceService(this.client)") &&
      importService.includes("new ReceiptService(this.client)"),
    "Import composition must remain mutation-only",
  );
  assert(
    !/this\.(?:invoiceService|receiptService)\.(?:listInvoices|listReceipts|getInvoiceById|getReceiptById|getUnallocatedReceipts)/
      .test(importService),
    "Import composition must not invoke user-domain reads",
  );
  assert(
    allocationService.includes("new ReceiptService(this.client)"),
    "Allocation composition retains an explicit mutation-only receipt service",
  );
  assert(
    !/this\.receiptService\.(?:listReceipts|getReceiptById|getUnallocatedReceipts)/
      .test(allocationService),
    "Allocation composition must not invoke user-domain receipt reads without JWT context",
  );
});
...
Deno.test("Batch 9D-D auto-allocation route remains disabled in source", async () => {
  ...
  assert(
    allocationsIndex.includes("AUTO_ALLOCATION_DISABLED"),
    "Expected disabled auto-allocation contract",
  );
  assert(
    allocationsIndex.includes("Automatic allocation route is disabled"),
    "Expected explicit disabled response",
  );
});
...
Deno.test("Batch 9D-D import contracts preserve provenance, explicit FX, and governance hold source paths", async () => {
  ...
  assert(
    importsService.includes(
      "function importOriginPayload(batch: ImportBatch, row: ImportRow)",
    ),
    "Expected trusted import-origin payload construction",
  );
  assert(
    importsService.includes(
      "const importOrigin = importOriginPayload(batch, row)",
    ),
    "Expected import-origin payload creation at execute time",
  );
  assert(
    importsService.includes(
      "this.invoiceService.createInvoice(auth, header, lines, { importOrigin })",
    ),
    "Expected invoice import-origin propagation to create service",
  );
  assert(
    importsService.includes(
      "this.receiptService.createReceipt(auth, receiptInput, { importOrigin })",
    ),
    "Expected receipt import-origin propagation to create service",
  );
  assert(
    importsService.includes("fields.exchange_rate"),
    "Expected explicit exchange_rate mapping preservation",
  );
  assert(
    importsService.includes("fields.fx_override_reason"),
    "Expected fx_override_reason mapping preservation",
  );
  assert(
    importsService.includes("HeldGovernance"),
    "Expected receipt explicit-FX governance hold state",
  );
  assert(
    importsService.includes("created_count: 0"),
    "Expected PDF/Image manual-review boundary without transaction creation",
  );
  assert(
    importsService.includes("posted_count: 0"),
    "Expected PDF/Image boundary without posting",
  );
  assert(
    importsService.includes("allocated_count: 0"),
    "Expected PDF/Image boundary without allocation",
  );
});
...
Deno.test("Batch 9D-D read contracts establish tenant and role scope before aggregation", async () => {
  ...
  assert(
    reportsService.includes("requireOperationalReadRole(auth)"),
    "Expected report reads to require operational read role",
  );
  assert(
    migration.includes("c.is_deleted = false") &&
      migration.includes("c.is_hidden = false"),
    "Expected hidden-customer filtering before SQL aggregation",
  );
  assert(
    migration.includes("public.user_customer_assignments"),
    "Expected AR Clerk assignment filtering before SQL aggregation",
  );
  assert(
    migration.includes("i.company_id = p_company_id"),
    "Expected company scope filters in report SQL",
  );
  assert(
    reportsService.includes("get_ar_dashboard_metrics"),
    "Expected dashboard to use the backend RPC contract",
  );
  assert(
    reportsService.includes("p_company_id: auth.companyId"),
    "Expected dashboard RPC tenant parameter",
  );
  assert(
    reportsService.includes("p_user_id: auth.userId"),
    "Expected dashboard RPC user parameter",
  );

  assert(
    allocationsService.includes(
      "const allowedReadRoles = ['AR Clerk', 'AR Supervisor', 'Finance Manager', 'Auditor']",
    ),
    "Expected allocation-history Auditor read access",
  );
  assert(
    allocationsService.includes("user_customer_assignments"),
    "Expected AR Clerk assignment scoping in allocation history",
  );
  assert(
    allocationsService.includes(".eq('company_id', auth.companyId)"),
    "Expected allocation-history company scoping",
  );
});
...
Deno.test("Batch 9D-D invoice and receipt read contracts expose additive FX governance summaries", async () => {
  ...
  for (const source of [invoiceService, receiptService]) {
    assert(
      source.includes("attachFxDecisionReadSummary"),
      "Expected FX decision read-summary enrichment",
    );
    assert(
      source.includes("fx_booking_rate_decisions"),
      "Expected decision summary to come from authoritative decision table",
    );
    assert(
      source.includes(
        "source_category, approval_status, lifecycle_status, decision_version, root_decision_id, supersedes_decision_id, import_origin, booked_rate, deviation_pct, stale_reference",
      ),
      "Expected authoritative decision fields in read contract",
    );
    assert(
      source.includes("fx_posting_eligibility"),
      "Expected narrowly named FX-governance eligibility summary",
    );
  }

  assert(
    invoiceService.includes("rpc('ar_invoice_collection'"),
    "Expected authoritative single-snapshot invoice collection RPC",
  );
  assert(
    receiptService.includes("rpc('ar_receipt_collection'"),
    "Expected authoritative single-snapshot receipt collection RPC",
  );

  assert(
    sharedTypes.includes("export interface FxDecisionReadSummary"),
    "Expected shared additive decision summary type",
  );
  assert(
    !sharedTypes.includes("fx_decision_status?: string | null"),
    "Unexpected temporary concatenated fx_decision_status field",
  );
  assert(
    sharedTypes.includes("base_available?: boolean"),
    "Expected optional additive base availability field",
  );
});
...
Deno.test("Batch 9D-D Migration 029 statically hardens governed sequence generation without changing callers", async () => {
  const migration029 = await read(
    "../../../../database/029_batch_9d_d_staging_runtime_defect_remediation.sql",
  );
  const migration028 = await read(
    "../../../../database/028_linked_credit_note_reference_integrity.sql",
  );
  const migration002 = await read("../../../../database/002_create_views.sql");
  const migration007 = await read(
    "../../../../database/007_financial_rpcs.sql",
  );
  const migration023 = await read(
    "../../../../database/023_fx_booking_rate_rpcs_and_immutability.sql",
  );
  ...
  assertEquals(
    (migration029.match(
      /CREATE OR REPLACE FUNCTION public\.get_next_sequence\(/g,
    ) ?? []).length,
    1,
  );
  ...
  const sequenceStart = migration029.indexOf(
  ...
  for (const documentType of ["CUST", "INV", "CN", "DN", "RCT", "JE"]) {
    assert(
      sequence.includes(`WHEN '${documentType}'`),
      `Missing ${documentType} numbering compatibility`,
    );
  }
  assert(sequence.includes("v_result := 'JE-' || p_source_type || '-'"));

  assert(
    migration028.includes(
      "v_je_no := public.get_next_sequence(p_company_id, 'JE', 'RCT');",
    ),
  );
  for (
    const sourceType of ["'INV'", "'CN'", "'DN'", "'RCT'", "'ADJ'", "'REV'"]
  ) {
    const callers = `${migration007}\n${migration023}\n${migration028}`;
    assert(
      callers.includes(sourceType),
      `Expected preserved ${sourceType} caller/source contract`,
    );
  }
  assert(sharedDb.includes("client.rpc('get_next_sequence'"));
  assert(migration002.includes("RETURNS VARCHAR(30)"));
  assert(
    !/GRANT\s+EXECUTE\s+ON\s+FUNCTION\s+public\.get_next_sequence/i.test(
      migration029,
    ),
  );
  assert(!/REVOKE\s+.*public\.get_next_sequence/i.test(migration029));

  const clearanceStart = migration028.indexOf(
    "CREATE FUNCTION public.clear_receipt_cheque(",
  );
  const clearanceEnd = migration028.indexOf("\n$$;", clearanceStart);
  const clearance = migration028.slice(clearanceStart, clearanceEnd);
  const sequenceCall = clearance.indexOf(
    "public.get_next_sequence(p_company_id, 'JE', 'RCT')",
  );
  const journalInsert = clearance.indexOf(
    "INSERT INTO public.journal_entries",
    sequenceCall,
  );
  const receiptUpdate = clearance.indexOf(
    "UPDATE public.receipts",
    journalInsert,
  );
  assert(
    sequenceCall >= 0 && journalInsert > sequenceCall &&
      receiptUpdate > journalInsert,
  );
  assert(
    !/^\s*COMMIT;/im.test(clearance),
    "Late failure must roll back sequence, journal, and Receipt mutations together",
  );
});
...
Deno.test("Batch 9D-D Migration 029 statically makes posted-reference target scope authorization-safe", async () => {
  ...
  assertEquals(
    (migration029.match(
      /CREATE OR REPLACE FUNCTION public\.correct_posted_invoice_reference\(/g,
    ) ?? []).length,
    1,
  );
  assert(migration029.includes(
  ...
  const accessCheck = body.indexOf(
  ...
  assert(
    roleCheck >= 0 && targetLock > roleCheck && forUpdate > targetLock &&
      accessCheck > forUpdate && referenceUpdate > accessCheck &&
      auditInsert > referenceUpdate,
  );

  assert(body.includes("WHEN SQLSTATE 'P0001' THEN"));
  assert(
    body.includes("GET STACKED DIAGNOSTICS v_access_message = MESSAGE_TEXT;"),
  );
  assert(body.includes("'AUTH: User does not have access to this customer'"));
  assert(body.includes("'NOT_FOUND: Customer not found'"));
  assertEquals(
    (body.match(/MESSAGE = 'NOT_FOUND: Financial document not found'/g) ?? [])
      .length,
    2,
  ...
  assert(
    body.includes("RAISE;"),
    "Unrelated P0001 errors must be re-raised unchanged",
  );
  assert(
    !body.includes("WHEN OTHERS"),
    "Unknown SQL errors must never be converted to NOT_FOUND",
  );
  assert(body.includes("SET reference_no = p_reference_no"));
  assert(body.includes("'Reference Correction'"));
  assert(body.includes("RETURN pg_catalog.to_jsonb(v_updated)"));
  assert(
    !/^\s*COMMIT;/im.test(body),
    "Reference update and audit insertion must remain one transaction",
  );
...
Deno.test("Batch 9D-D Migration 029 hidden and nonexistent reference targets have an identical production-handler envelope", async () => {
  ...
  const invoke = async (
    ...
    assertEquals(client.rpcCalls.length, 1);
    assertEquals(
      (client.rpcCalls[0].params as MockRow).p_invoice_id,
      invoiceId,
    );
  ...
  assertJsonEquals(
    hidden.body,
    nonexistent.body,
    "Hidden and nonexistent envelopes must be byte-equivalent JSON",
  );
...
Deno.test("Gate D legacy v1 mapper keeps its internal tag out of Invoice and Receipt API envelopes", () => {
  ...
  for (
    const [raw, currentAmountBasis] of [
      [
        build(CURRENT_OUTSTANDING_AMOUNT_BASIS),
        CURRENT_OUTSTANDING_AMOUNT_BASIS,
      ],
      [
        build(CURRENT_UNALLOCATED_AMOUNT_BASIS),
        CURRENT_UNALLOCATED_AMOUNT_BASIS,
      ],
    ] as const
  ) {
...
Deno.test("Gate D v2 Invoice and Receipt summaries retain multi-currency and Gate C export contracts", async () => {
  ...
  assertEquals(
    invoice.current_balance_summary.by_currency[1].base_amount,
    null,
  );
  assertEquals(
    receipt.current_balance_summary.by_currency[1].base_amount,
    null,
  );
  ...
  for (
    ...
    assert(
      migration033.includes(field),
      `Gate D must retain Dashboard field ${field}`,
    );
  }

  const exportService = await Deno.readTextFile(
    new URL("./export-service.ts", import.meta.url),
  );
